...
conn_type_t ct_memcache_client = {.magic = CONN_FUNC_MAGIC,
                                  .flags = 0,
...
struct memcache_client_functions default_memcache_client = {.info = NULL,
                                                            .execute = mcc_execute,
      ...
      switch (c->parse_state) {
        ...
        case mrp_readcommand:
          ...
          switch (D->comm[0]) {
            ...
            case 'S':
              ...
              } else if (!strcmp(D->comm, "SERVER_ERROR")) {
            ...
            case 'N':
              ...
              } else if (!strcmp(D->comm, "NOT_FOUND")) {
        ...
        case mrp_readint:
          ...
          while (ptr < ptr_e && *ptr >= '0' && *ptr <= '9') {
            if (tt > 0x7fffffffffffffffLL / 10
                || (tt == 0x7fffffffffffffffLL / 10 && *ptr > '0' + (char)(0x7fffffffffffffffLL % 10) + ((D->response_flags & 2) != 0))) {
...
int mcc_init_crypto(struct connection *c) {
  ...
  write_out(&c->Out, buf,
            snprintf(buf, buf_size, "delete @#$AuTh$#@A:%08x:%016llx%016llx\r\n", MCC_DATA(c)->nonce_time, *(long long *)MCC_DATA(c)->nonce,
