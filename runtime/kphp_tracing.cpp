...
TracingContext& TracingContext::get() noexcept { return tracing_ctx; }

int generate_uniq_id() { return f$mt_rand(); }
...
static void fill_BuiltinFuncID_strings(array<string>& out) noexcept {
  auto add = [&out](BuiltinFuncID func_id, const char* title) { out.set_value(static_cast<int64_t>(func_id), string(title, strlen(title))); };
  auto add_branch = [&out](BuiltinFuncID func_id, BuiltinFuncID branch_num, const char* title) {
    out.set_value(static_cast<int64_t>(func_id) + (static_cast<int64_t>(branch_num) << static_cast<int>(BuiltinFuncID::_shift_for_branch)),
                  string(title, strlen(title)));
  };
...
static inline double calc_now_timestamp() noexcept { return std::chrono::duration<double>{std::chrono::system_clock::now().time_since_epoch()}.count(); }

static inline float calc_time_offset(double now_timestamp) noexcept { return static_cast<float>(now_timestamp - kphp_tracing::cur_start_timestamp); }

static inline int calc_coroutine_id(int64_t fork_id) noexcept { return fork_id > 0 ? static_cast<int>(fork_id - first_forked_resumable_id + 1) : 0; }
...
static inline uint64_t encode_trace_context_int2(int div_id, int trace_flags) { return (static_cast<uint64_t>(div_id) << 32) + trace_flags; }
...
void on_curl_add_attribute(int curl_handle, const string& key, const string& value) { BinlogWriter::onCurlAddedAttributeString(curl_handle, key, value); }

void on_curl_add_attribute(int curl_handle, const string& key, int value) { BinlogWriter::onCurlAddedAttributeInt32(curl_handle, key, value); }
...
void on_php_script_warning(const string& warning_message) { BinlogWriter::provideRuntimeWarning(0, warning_message); }
...
void C$KphpDiv::to_json(char* buf, size_t buf_len) const {
  ...
  snprintf(buf, buf_len, R"({"trace_ctx":"%016)" PRIx64 R"(%016)" PRIx64 R"(","name":"%s:%s","start":%.3f,"dur":%.3f})", trace_id, trace_ctx_int2,
           str_to_json(root_span_title).c_str(), str_to_json(root_span_desc).c_str(), start_timestamp, end_timestamp - start_timestamp);
}

double f$KphpDiv$$getStartTimestamp(const class_instance<C$KphpDiv>& v$this) { return v$this->start_timestamp; }

double f$KphpDiv$$getEndTimestamp(const class_instance<C$KphpDiv>& v$this) { return v$this->end_timestamp; }
...
void C$KphpSpan::addAttributeString(const string& key, const string& value) const { BinlogWriter::onSpanAddedAttributeString(span_id, key, value); }
...
void C$KphpSpan::addAttributeFloat(const string& key, double value) const { BinlogWriter::onSpanAddedAttributeFloat64(span_id, key, value); }

void C$KphpSpan::addAttributeBool(const string& key, bool value) const { BinlogWriter::onSpanAddedAttributeBool(span_id, key, value); }
...
void C$KphpSpan::addLink(const class_instance<C$KphpSpan>& another) const { BinlogWriter::onSpanAddedLink(span_id, another->span_id); }
...
void C$KphpSpan::exclude() const { BinlogWriter::onSpanExcluded(span_id); }
...
void C$KphpSpanEvent::addAttributeString(const string& key, const string& value) const { BinlogWriter::onSpanAddedAttributeString(span_id, key, value); }
...
void C$KphpSpanEvent::addAttributeFloat(const string& key, double value) const { BinlogWriter::onSpanAddedAttributeFloat64(span_id, key, value); }

void C$KphpSpanEvent::addAttributeBool(const string& key, bool value) const { BinlogWriter::onSpanAddedAttributeBool(span_id, key, value); }
...
int64_t f$kphp_tracing_get_level() { return kphp_tracing::TracingContext::get().cur_trace_level; }
