...
struct tl_exclamation_fetch_wrapper {
  ...
  explicit tl_exclamation_fetch_wrapper(std::unique_ptr<tl_func_base> fetcher) : fetcher(std::move(fetcher)) {}

  tl_exclamation_fetch_wrapper() = default;
  tl_exclamation_fetch_wrapper(tl_exclamation_fetch_wrapper&&) = default;

  mixed fetch() { return fetcher->fetch(); }
...
struct t_Long {
  void store(const mixed& tl_object) { store_long(tl_object); }
  ...
  void typed_store(const PhpType& v) { f$store_long(v); }
...
struct t_Double {
  void store(const mixed& tl_object) { f$store_double(f$floatval(tl_object)); }
  ...
  void typed_store(const PhpType& v) { f$store_double(v); }
...
struct t_Float {
  void store(const mixed& tl_object) { f$store_float(f$floatval(tl_object)); }
  ...
  void typed_store(const PhpType& v) { f$store_float(v); }
...
struct t_String {
  void store(const mixed& tl_object) { f$store_string(f$strval(tl_object)); }
  ...
  void typed_store(const PhpType& v) { f$store_string(f$strval(v)); }
...
struct t_Bool {
  void store(const mixed& tl_object) { store_int(tl_object.to_bool() ? TL_BOOL_TRUE : TL_BOOL_FALSE); }
  ...
  void typed_store(const PhpType& v) { store_int(v ? TL_BOOL_TRUE : TL_BOOL_FALSE); }
...
struct t_Vector {
  ...
  explicit t_Vector(T param_type) : elem_state(std::move(param_type)) {}
...
struct t_Maybe {
  ...
  explicit t_Maybe(T param_type) : elem_state(std::move(param_type)) {}
  ...
  static bool has_maybe_value(const PhpType& v) { return !v.is_null(); }
...
struct tl_Dictionary_impl {
  ...
  explicit tl_Dictionary_impl(ValueT value_type) : value_state(std::move(value_type)) {}
...
struct t_Tuple {
  ...
  t_Tuple(T param_type, int64_t size) : elem_state(std::move(param_type)), size(size) {}
...
struct tl_array {
  ...
  tl_array(int64_t size, T cell) : size(size), cell(std::move(cell)) {}
