...
const string_buffer *zlib_encode(const char *s, int32_t s_len, int32_t level, int32_t encoding) {
  ...
  vk::singleton<KphpRuntimeContext>::get().static_SB.clean().reserve(res_len);
  ...
  if (ret == Z_OK) {
    ...
    strm.next_out = reinterpret_cast <Bytef *> (vk::singleton<KphpRuntimeContext>::get().static_SB.buffer());
    ...
    if (ret == Z_STREAM_END) {
      ...
      vk::singleton<KphpRuntimeContext>::get().static_SB.set_pos(static_cast<int64_t>(strm.total_out));
      return &vk::singleton<KphpRuntimeContext>::get().static_SB;
    }
  }
  dl::leave_critical_section();
