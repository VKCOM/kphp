...
void zlib_dynamic_free(voidpf opaque __attribute__((unused)), voidpf address) { dl::script_allocator_free(address); }
} // namespace
...
string f$gzdecode(const string& s) { return zlib_decode(s, ZLIB_ENCODING_GZIP); }

string f$gzuncompress(const string& s) { return zlib_decode(s, ZLIB_ENCODING_DEFLATE); }

string f$gzinflate(const string& s) { return zlib_decode(s, ZLIB_ENCODING_RAW); }
