...
var stdClass::call (const string &name, const array <var> &params __attribute__((unused))) {
...
template <class T>
object_ptr <T>::object_ptr (void): o (static_cast <T *> (dl::allocate (sizeof (T)))) {
  new (o) T();
}
...
template <class T>
T& object_ptr <T>::operator *() {
  return *o;
}

template <class T>
const T& object_ptr <T>::operator *() const {
  return *o;
}
