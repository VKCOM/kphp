...
bool f$checkdate(int64_t month, int64_t day, int64_t year) { return php_timelib_is_valid_date(month, day, year); }
...
string f$date_default_timezone_get() { return string(default_timezone_id); }
...
void f$set_use_updated_gmmktime(bool enable) { use_updated_gmmktime = enable; }
...
void free_use_updated_gmmktime() { use_updated_gmmktime = false; }
...
array<mixed> f$date_parse(const string& time_str) { return php_timelib_date_parse(time_str); }

array<mixed> f$date_parse_from_format(const string& format, const string& time_str) { return php_timelib_date_parse_from_format(format, time_str); }

int64_t f$time() { return time(nullptr); }

int64_t hrtime_int() { return std::chrono::steady_clock::now().time_since_epoch().count(); }
