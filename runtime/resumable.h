...
public:
  ...
  virtual bool is_internal_resumable() const noexcept { return false; }
...
public:
  wait_result_resumable(int64_t resumable_id, double timeout) noexcept : resumable_id_(resumable_id), timeout_(timeout) {}
...
public:
  explicit wait_multi_resumable(const array<int64_t>& resumable_ids) noexcept : resumable_ids_(resumable_ids), result_(resumable_ids.size()) {}
