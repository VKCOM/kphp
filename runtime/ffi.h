...
struct CDataArray : public refcountable_php_classes<CDataArray<T>> {
  ...
  explicit CDataArray(int64_t len) : data{static_cast<T*>(dl::allocate(sizeof(T) * len))}, len{len} {
    static_assert(std::is_trivial_v<T>, "CDataArray elements should be trivial");
    std::memset(data, 0, sizeof(T) * len);
  }

  CDataArray(T* data, int64_t len) : data{data}, len{len} {}
...
struct CDataPtr {
  ...
  bool is_php_null() const noexcept { return c_value == reinterpret_cast<T*>(1); }
...
public:
  const char* get_class() const noexcept { return "FFI\\Scope"; }
};
...
inline int64_t f$ffi_cast_ptr2addr(CDataPtr<void> ptr) { return reinterpret_cast<int64_t>(ptr.c_value); }
...
inline const char* ffi_php2c(const Optional<string>& v, ffi_tag<const char*>) { return v.has_value() ? v.val().c_str() : nullptr; }
