...
inline void f$echo(const string& s) { print(s); }

inline void f$dbg_echo(const string& s) { dbg_echo(s); }
...
inline void f$set_json_log_on_timeout_mode(bool enabled) { is_json_log_on_timeout_enabled = enabled; }

extern bool is_demangled_stacktrace_logs_enabled;

inline void f$set_json_log_demangle_stacktrace(bool enable) { is_demangled_stacktrace_logs_enabled = enable; }

inline void f$kphp_turn_on_host_tag_in_inner_statshouse_metrics_toggle() { StatsHouseManager::get().turn_on_host_tag_toggle(); }
