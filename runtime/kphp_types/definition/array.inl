...
typename array<T>::array_inner *array<T>::array_inner::create(int64_t new_int_size, bool is_vector) {
  ...
  if (is_vector) {
    auto p = reinterpret_cast<array_inner *>(platform::allocate(mem_size));
  ...
  array_inner *p = shift_pointer_to_array_inner(platform::allocate0(mem_size));
  ...
  if (ref_cnt < ExtraRefCnt::for_global_const) {
    ...
    if (ref_cnt <= -1) {
      if (is_vector()) {
        ...
        platform::deallocate((void *)this, sizeof_vector(buf_size));
        return;
      }
      ...
      platform::deallocate(shifted_this, sizeof_map(buf_size));
    }
  }
}
...
void array<T>::mutate_to_size(int64_t int_size) {
  ...
  p = static_cast<array_inner *>(platform::reallocate(p, p->sizeof_vector(new_int_buff_size), p->sizeof_vector(p->buf_size)));
  ...
  if (is_vector()) {
    if (other.is_vector()) {
      ...
      if (p->ref_cnt > 0) {
        ...
        p = (array_inner *)platform::reallocate((void *)p, p->sizeof_vector(new_size), p->sizeof_vector(p->buf_size));
        p->buf_size = new_size;
      }
...
void array<T>::sort(const T1 &compare, bool renumber) {
  ...
  array_bucket **arTmp = (array_bucket **)platform::allocate(n * sizeof(array_bucket * ));
  uint32_t i = 0;
  for (array_bucket *it = p->begin(); it != p->end(); it = p->next(it)) {
    arTmp[i++] = it;
  }
  ...
  platform::deallocate(arTmp, n * sizeof(array_bucket * ));
}
