...
static const stream_functions* get_stream_functions(const string& name) { return wrappers.get_value(name); }
...
Stream f$fopen(const string& stream, const string& mode) { STREAM_FUNCTION_BODY(fopen, false)(url, mode); }

Optional<int64_t> f$fwrite(const Stream& stream, const string& text) { STREAM_FUNCTION_BODY(fwrite, false)(stream, text); }

int64_t f$fseek(const Stream& stream, int64_t offset, int64_t whence) { STREAM_FUNCTION_BODY(fseek, -1)(stream, offset, whence); }

bool f$rewind(const Stream& stream) { return f$fseek(stream, 0, 0) == 0; }

Optional<int64_t> f$ftell(const Stream& stream) { STREAM_FUNCTION_BODY(ftell, false)(stream); }

Optional<string> f$fread(const Stream& stream, int64_t length) { STREAM_FUNCTION_BODY(fread, false)(stream, length); }

Optional<string> f$fgetc(const Stream& stream) { STREAM_FUNCTION_BODY(fgetc, false)(stream); }

Optional<string> f$fgets(const Stream& stream, int64_t length) { STREAM_FUNCTION_BODY(fgets, false)(stream, length); }

Optional<int64_t> f$fpassthru(const Stream& stream) { STREAM_FUNCTION_BODY(fpassthru, false)(stream); }

bool f$fflush(const Stream& stream) { STREAM_FUNCTION_BODY(fflush, false)(stream); }

bool f$feof(const Stream& stream) { STREAM_FUNCTION_BODY(feof, true)(stream); }

bool f$fclose(const Stream& stream) { STREAM_FUNCTION_BODY(fclose, false)(stream); }

Optional<int64_t> f$fprintf(const Stream& stream, const string& format, const array<mixed>& args) { return f$vfprintf(stream, format, args); }
...
Optional<string> f$file_get_contents(const string& stream) { STREAM_FUNCTION_BODY(file_get_contents, false)(url); }
...
void init_streams_lib() { reset_streams_global_vars(); }

void free_streams_lib() { reset_streams_global_vars(); }
