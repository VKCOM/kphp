...
struct array_sv {
  explicit array_sv(Visitor& visitor) noexcept : visitor_(visitor) {}
  bool operator()(uint32_t size) const { return visitor_.start_array(size); }
  container_type type() const { return container_type::ARRAY_ITEM; }
...
template <typename Visitor>
struct array_ev {
  explicit array_ev(Visitor& visitor) noexcept : visitor_(visitor) {}
  bool operator()() const { return visitor_.end_array(); }
...
template <typename Visitor>
struct map_sv {
  explicit map_sv(Visitor& visitor) noexcept : visitor_(visitor) {}
  bool operator()(uint32_t size) const { return visitor_.start_map(size); }
  container_type type() const { return container_type::MAP_KEY; }
...
template <typename Visitor>
struct map_ev {
  explicit map_ev(Visitor& visitor) noexcept : visitor_(visitor) {}
  bool operator()() const { return visitor_.end_map(); }
...
struct unpack_stack {
  struct stack_elem {
    stack_elem(container_type type, uint32_t rest) noexcept : m_type(type), m_rest(rest) {}
...
template <typename Visitor>
parser<Visitor>::parser(Visitor& visitor, unpack_stack& stack) noexcept : visitor_(visitor), stack_(stack), m_cs(msgpack_cs::HEADER) {}
