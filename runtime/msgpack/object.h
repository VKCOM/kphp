...
inline msgpack::object const &adaptor::convert<T, Enabler>::operator()(msgpack::object const &o, T &v) const {
  v.msgpack_unpack(o);
  return o;
}
...
inline msgpack::packer<Stream> &adaptor::pack<T, Enabler>::operator()(msgpack::packer<Stream> &o, T const &v) const {
...
template<typename T>
inline std::enable_if_t<!std::is_array_v<T> && !std::is_pointer_v<T>, T &> object::convert(T &v) const {
