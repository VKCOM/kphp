...
public:
  explicit EasyContext(int64_t self_handler_id) noexcept : self_id(self_handler_id) {}
...
void register_curl_deallocation(size_t memory_used) noexcept { vk::singleton<CurlMemoryUsage>::get().currently_allocated -= memory_used; }
...
void global_init_curl_lib() noexcept {
  if (curl_global_init_mem(
          ...
          [](size_t nmemb, size_t size) { return register_curl_allocation(calloc(nmemb, size)); }) != CURLE_OK) {
...
CurlRequest::CurlRequest(curl_easy easy_id, curl_multi multi_id) noexcept
    : easy_id(easy_id), multi_id(multi_id), request_id(curl_requests_factory.create_slot()) {}
