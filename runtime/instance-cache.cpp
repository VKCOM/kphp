...
public:
  ...
  ElementHolder(std::chrono::nanoseconds now, int64_t ttl, std::unique_ptr<InstanceCopyistBase>&& instance, CacheContext& context) noexcept
      : inserted_by_process(getpid()), instance_wrapper(std::move(instance)), cache_context(context) {
...
struct SharedDataStorages : private vk::not_copyable {
  explicit SharedDataStorages(memory_resource::unsynchronized_pool_resource& resource) : storage(ElementStorage_::allocator_type{resource}) {}
...
public:
  ...
  SharedDataStorages* get_data_shards() noexcept { return data_shards_; }

  static constexpr size_t get_data_shards_count() noexcept { return DATA_SHARDS_COUNT; }
...
private:
  ...
  static constexpr size_t get_context_size() noexcept { return (sizeof(CacheContext) + 7) & -8; }

  static constexpr size_t get_data_size() noexcept { return (sizeof(SharedDataStorages) * DATA_SHARDS_COUNT + 7) & -8; }
...
class InstanceCache {
private:
  InstanceCache() : now_(std::chrono::system_clock::now().time_since_epoch()) {}
...
public:
  ...
  void update_now() { now_ = std::chrono::system_clock::now().time_since_epoch(); }
  ...
  void force_release_all_resources() { data_manager_.force_release_all_resources(); }
  ...
  // this function should be called only from master
  const InstanceCacheStats& get_stats() { return data_manager_.get_current_resource().get_context().stats; }

  // this function should be called only from master
  const memory_resource::MemoryStats& get_last_memory_stats() const noexcept { return last_memory_stats_; }
...
private:
  ...
  struct IntrusivePtrHash {
    size_t operator()(const vk::intrusive_ptr<ElementHolder>& el) const { return reinterpret_cast<size_t>(el.get()); }
  };
...
const InstanceCopyistBase* instance_cache_fetch_wrapper(const string& key, bool even_if_expired) { return InstanceCache::get().fetch(key, even_if_expired); }

} // namespace impl_

void global_init_instance_cache_lib() { impl_::InstanceCache::get().global_init(); }

void init_instance_cache_lib() { impl_::InstanceCache::get().refresh(); }

void free_instance_cache_lib() { impl_::InstanceCache::get().free(); }

// should be called only from master
void set_instance_cache_memory_limit(size_t limit) { impl_::instance_cache_settings.total_memory_limit = limit; }

// should be called only from master
InstanceCacheSwapStatus instance_cache_try_swap_memory() { return impl_::InstanceCache::get().try_swap_memory_resource(); }

// should be called only from master
const InstanceCacheStats& instance_cache_get_stats() { return impl_::InstanceCache::get().get_stats(); }

// should be called only from master
const memory_resource::MemoryStats& instance_cache_get_memory_stats() { return impl_::InstanceCache::get().get_last_memory_stats(); }

// should be called only from master
void instance_cache_purge_expired_elements() { impl_::InstanceCache::get().purge_expired(); }

void instance_cache_release_all_resources_acquired_by_this_proc() { impl_::InstanceCache::get().force_release_all_resources(); }

bool f$instance_cache_update_ttl(const string& key, int64_t ttl) { return impl_::InstanceCache::get().update_ttl(key, ttl); }

bool f$instance_cache_delete(const string& key) { return impl_::InstanceCache::get().del(key); }
