...
unsigned int kphp_ml::MLModel::calculate_mutable_buffer_size() const {
  ...
  case ModelKind::catboost_trees: {
    const auto& cbm = std::get<kphp_ml_catboost::CatboostModel>(impl);
    return cbm.cat_feature_count * sizeof(string) + cbm.float_feature_count * sizeof(float) +
           (cbm.binary_feature_count + 4 - 1) / 4 * 4 + // round up to 4 bytes
           cbm.cat_feature_count * sizeof(int) + cbm.model_ctrs.used_model_ctrs_count * sizeof(float);
  }
  default:
    return 0;
  }
...
const std::vector<std::string>& kphp_ml::MLModel::get_feature_names() const { return feature_names; }
