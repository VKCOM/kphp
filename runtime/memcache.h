...
public:
  ...
  void accept(CommonMemoryEstimateVisitor& visitor [[maybe_unused]]) final { visitor("", array<int64_t>{}); }

  const char* get_class() const final { return "McMemcache"; }

  int32_t get_hash() const final { return static_cast<int32_t>(vk::std_hash(vk::string_view(C$McMemcache::get_class()))); }

  virtual C$McMemcache* virtual_builtin_clone() const noexcept { return new C$McMemcache{*this}; }

  virtual size_t virtual_builtin_sizeof() const noexcept { return sizeof(*this); }
