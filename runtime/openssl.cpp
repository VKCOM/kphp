...
public:
  ...
  string hash(const string& s, bool raw_output) const noexcept {
    return call_hash_algo(raw_output, [this, &s](string& out) {
      dl::critical_section_call(algo_function, reinterpret_cast<const unsigned char*>(s.c_str()), s.size(), reinterpret_cast<unsigned char*>(out.buffer()));
    });
  }
  ...
  string hash_hmac(const string& data, const string& key, bool raw_output) const noexcept {
    return call_hash_algo(raw_output, [this, &data, &key](string& out) {
      ...
      dl::critical_section_call(HMAC, get_evp(), key.c_str(), static_cast<int>(key.size()), reinterpret_cast<const unsigned char*>(data.c_str()),
                                static_cast<int>(data.size()), reinterpret_cast<unsigned char*>(out.buffer()), &md_len);
      php_assert(md_len == hash_len);
    });
  }
};

HashTraits make_sha1_traits() noexcept { return HashTraits{"sha1", SHA1, SHA_DIGEST_LENGTH, EVP_sha1}; }

HashTraits make_md5_traits() noexcept { return HashTraits{"md5", MD5, MD5_DIGEST_LENGTH, EVP_md5}; }

const auto& get_supported_hash_algorithms() noexcept {
  const static auto supported_algorithms = vk::to_array<HashTraits>(
      {make_sha1_traits(), HashTraits{"sha224", SHA224, SHA224_DIGEST_LENGTH, EVP_sha224}, HashTraits{"sha256", SHA256, SHA256_DIGEST_LENGTH, EVP_sha256},
       HashTraits{"sha384", SHA384, SHA384_DIGEST_LENGTH, EVP_sha384}, HashTraits{"sha512", SHA512, SHA512_DIGEST_LENGTH, EVP_sha512}, make_md5_traits()});
...
const HashTraits& find_hash_algorithm(const char* algo) noexcept {
  ...
  const auto* const it =
      std::find_if(supported_algorithms.begin(), supported_algorithms.end(), [algo](const HashTraits& traits) { return !strcmp(algo, traits.name); });
...
array<string> f$hash_hmac_algos() noexcept { return f$hash_algos(); }

string f$hash(const string& algo, const string& s, bool raw_output) noexcept { return find_hash_algorithm(algo.c_str()).hash(s, raw_output); }
...
string f$sha1(const string& s, bool raw_output) noexcept { return make_sha1_traits().hash(s, raw_output); }

string f$md5(const string& s, bool raw_output) noexcept { return make_md5_traits().hash(s, raw_output); }
...
int64_t f$crc32(const string& s) { return compute_crc32(static_cast<const void*>(s.c_str()), s.size()); }
...
public:
  ...
  EVP_PKEY* find_resource(const string& key) {
    php_assert(registered_keys_);
    int64_t num = 0;
    if (key.size() > 2 && key[0] == PREFIX_CHAR && key[1] == PREFIX_CHAR && php_try_to_int(key.c_str() + 2, key.size() - 2, &num)) {
...
static EVP_PKEY* openssl_get_public_evp(const string& key, bool& from_cache) {
  ...
  X509* cert = reinterpret_cast<X509*>(PEM_ASN1_read_bio(reinterpret_cast<d2i_of_void*>(d2i_X509), PEM_STRING_X509, cert_in, nullptr, nullptr, nullptr));
  BIO_free(cert_in);
  if (cert) {
...
bool f$openssl_sign(const string& data, string& signature, const string& priv_key_id, int64_t algo) {
  ...
  if (md_ctx && EVP_SignInit(md_ctx, mdtype) && EVP_SignUpdate(md_ctx, data.c_str(), data.size()) &&
...
int64_t f$openssl_verify(const string& data, const string& signature, const string& pub_key_id, int64_t algo) {
  ...
  int err = 0;
  EVP_MD_CTX* md_ctx = EVP_MD_CTX_create();
  if (md_ctx == nullptr || !EVP_VerifyInit(md_ctx, mdtype) || !EVP_VerifyUpdate(md_ctx, data.c_str(), data.size()) ||
...
static Stream ssl_stream_socket_client(const string& url, int64_t& error_number, string& error_description, double timeout,
                                       int64_t flags __attribute__((unused)), const mixed& options) {
#define RETURN(dump_error_stack)                                                                                                                               \
  if (dump_error_stack) {                                                                                                                                      \
    php_warning("%s: %s", error_description.c_str(), ssl_get_error_string());                                                                                  \
  } else {                                                                                                                                                     \
...
#define RETURN_ERROR_FORMAT(dump_error_stack, error_no, format, ...)                                                                                           \
  error_number = error_no;                                                                                                                                     \
  error_description = f$sprintf(CONST_STRING(format), array<mixed>::create(__VA_ARGS__));                                                                      \
  RETURN(dump_error_stack)
...
static bool ssl_context_set_option(mixed& context_ssl, const string& option, const mixed& value) {
  if (STRING_EQUALS(option, "verify_peer") || STRING_EQUALS(option, "verify_depth") || STRING_EQUALS(option, "cafile") || STRING_EQUALS(option, "capath") ||
      STRING_EQUALS(option, "local_cert") || STRING_EQUALS(option, "ciphers")) {
...
public:
  explicit X509_parser(const string& data) noexcept : x509_(get_x509_from_data(data)) {}
  ...
  Optional<array<mixed>> parse(bool shortnames = true) const noexcept {
    ...
    return std::initializer_list<std::pair<string, mixed>>{{string("name"), get_subject_name()},
  ...
  static void free_leaked_x509() noexcept { processing_x509_.reset(); }
...
Optional<array<mixed>> f$openssl_x509_parse(const string& data, bool shortnames /* = true */) { return X509_parser{data}.parse(shortnames); }

int64_t f$openssl_x509_verify(const string& signature, const string& public_key) { return X509_parser{signature}.verify(public_key); }

mixed f$openssl_x509_checkpurpose(const string& data, int64_t purpose) { return X509_parser{data}.check_purpose(purpose); }
...
X509_ptr read_x509_cert(const string& cert_str) noexcept {
  ...
  if (vk::string_view{cert_str.c_str(), cert_str.size()}.starts_with(file_prefilx)) {
    ...
    return X509_ptr{static_cast<X509*>(PEM_ASN1_read_bio(reinterpret_cast<d2i_of_void*>(d2i_X509), PEM_STRING_X509, in.get(), nullptr, nullptr, nullptr))};
  }
  return {};
}
...
bool f$openssl_pkcs7_sign(const string& infile, const string& outfile, const string& sign_cert, const string& priv_key, const array<string>& headers,
                          int64_t flags, const string& extra_certs) noexcept {
  dl::CriticalSectionGuard critical_section;
  ...
  for (auto header : headers) {
    const int ret = header.is_string_key() ? BIO_printf(outfile_bio.get(), "%s: %s\n", header.get_string_key().c_str(), header.get_value().c_str())
...
enum cipher_opts : int { OPENSSL_RAW_DATA = 1, OPENSSL_ZERO_PADDING = 2, OPENSSL_DONT_ZERO_PAD_KEY = 4 };

struct CipherCtx {
  enum cipher_action { decrypt = 0, encrypt = 1 };

  CipherCtx(const string& method, int64_t options, cipher_action action) : options_(options), action_(action) {
    dl::CriticalSectionGuard critical_section;
  ...
  string flush_result() { return std::move(out_); };
  ...
  bool init(string key, string iv, string& tag) {
    ...
    if (!EVP_CipherInit_ex(ctx_, nullptr, nullptr, reinterpret_cast<const unsigned char*>(key.c_str()), reinterpret_cast<const unsigned char*>(iv.c_str()),
                           action_)) {
      php_warning("Cipher init error:\n%s", ssl_get_error_string());
      return false;
    }
  ...
  bool update(const string& data, const string& aad) {
    ...
    critical_section.enter_critical_section();
    if (!EVP_CipherUpdate(ctx_, reinterpret_cast<unsigned char*>(out_.buffer()), &out_len_, reinterpret_cast<const unsigned char*>(data.c_str()),
  ...
  bool finalize() {
    ...
    const int is_ok = dl::critical_section_call([&] {
      return action_ == encrypt ? EVP_EncryptFinal(ctx_, reinterpret_cast<unsigned char*>(out_.buffer()) + out_len_, &i)
...
Optional<string> f$openssl_encrypt(const string& data, const string& method, const string& key, int64_t options, const string& iv, string& tag,
  ...
  return (options & OPENSSL_RAW_DATA) ? result : f$base64_encode(result.val());
}
