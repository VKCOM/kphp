...
void* allocate(size_t size) noexcept {
  ...
  if (unlikely(!script_allocator_enabled)) {
    php_runtime_critical("Trying to call allocate for non runned script, n = %zu", size);
    return nullptr;
  }
...
void* allocate0(size_t size) noexcept {
  ...
  if (unlikely(!script_allocator_enabled)) {
    php_runtime_critical("Trying to call allocate0 for non runned script, n = %zu", size);
    return nullptr;
  }
...
void* reallocate(void* mem, size_t new_size, size_t old_size) noexcept {
  ...
  if (unlikely(!script_allocator_enabled)) {
    php_runtime_critical("Trying to call reallocate for non runned script, p = %p, new_size = %zu, old_size = %zu", mem, new_size, old_size);
    return mem;
  }
...
void* operator new(size_t size) {
  ...
  if (!res) {
    php_runtime_critical("nullptr from malloc");
  }
  return res;
}
