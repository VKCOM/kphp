...
public:
  using ReturnT = bool;
  explicit ExecuteResumable(PgsqlPdoEmulatedStatement* ctx, int64_t timeout_sec) noexcept : ctx(ctx), timeout_sec(timeout_sec) {}
...
PgsqlPdoEmulatedStatement::PgsqlPdoEmulatedStatement(const string& statement, int connector_id) : statement(statement), connector_id(connector_id) {}
...
int64_t PgsqlPdoEmulatedStatement::affected_rows() noexcept { return response->affected_rows; }
} // namespace pdo::pgsql
