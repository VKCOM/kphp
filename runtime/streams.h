...
struct stream_functions {
  ...
  bool (*context_set_option) (const var &context, const string &option, const var &value);
};
...
extern int error_number_dummy;
extern string error_description_dummy;

const int STREAM_CLIENT_CONNECT = 1;

var f$stream_socket_client (const string &url, int &error_number = error_number_dummy, string &error_description = error_description_dummy, double timeout = -1, int flags = STREAM_CLIENT_CONNECT, const var &context = var());
