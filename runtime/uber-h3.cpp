...
inline std::tuple<double, double> coord2deg(GeoCoord geo_coord) noexcept { return std::make_tuple(radsToDegs(geo_coord.lat), radsToDegs(geo_coord.lon)); }
...
public:
  GeoPolygonOwner(const array<std::tuple<double, double>>& polygon_boundary, const array<array<std::tuple<double, double>>>& holes) noexcept
      : polygon_boundary_(array_size{polygon_boundary.count(), true}), holes_(array_size{holes.count(), true}) {
  ...
  const GeoPolygon& getPolygon() const noexcept { return polygon; }
...
int64_t f$UberH3$$h3GetResolution(int64_t h3_index) noexcept { return h3GetResolution(static_cast<H3Index>(h3_index)); }

int64_t f$UberH3$$h3GetBaseCell(int64_t h3_index) noexcept { return h3GetBaseCell(static_cast<H3Index>(h3_index)); }

int64_t f$UberH3$$stringToH3(const string& h3_index_str) noexcept { return stringToH3(h3_index_str.c_str()); }
...
bool f$UberH3$$h3IsValid(int64_t h3_index) noexcept { return h3IsValid(static_cast<H3Index>(h3_index)) != 0; }

bool f$UberH3$$h3IsResClassIII(int64_t h3_index) noexcept { return h3IsResClassIII(static_cast<H3Index>(h3_index)) != 0; }

bool f$UberH3$$h3IsPentagon(int64_t h3_index) noexcept { return h3IsPentagon(static_cast<H3Index>(h3_index)) != 0; }
...
int64_t f$UberH3$$maxFaceCount(int64_t h3_index) noexcept { return maxFaceCount(static_cast<H3Index>(h3_index)); }
