...
bool mc_is_immediate_query(const string& key) { return key[0] == '^' && key.size() >= 2; }
...
C$McMemcache::host::host() : host_num(-1), host_port(-1), host_weight(0), timeout_ms(200) {}

C$McMemcache::host::host(int32_t host_num, int32_t host_port, int32_t host_weight, int32_t timeout_ms)
    : host_num(host_num), host_port(host_port), host_weight(host_weight), timeout_ms(timeout_ms) {}
...
class_instance<C$McMemcache> f$McMemcache$$__construct(const class_instance<C$McMemcache>& v$this) { return v$this; }
...
void init_memcache_lib() { reset_drivers_global_vars(); }

void free_memcache_lib() { reset_drivers_global_vars(); }
