...
mixed f$kphp_get_runtime_config() { return runtime_config; }

void f$ob_clean() { coub->clean(); }
...
string f$ob_get_contents() { return coub->str(); }
...
int64_t f$ob_get_level() { return ob_cur_buffer; }
...
shutdown_functions_status get_shutdown_functions_status() { return shutdown_functions_status_value; }

int get_shutdown_functions_count() { return shutdown_functions_count; }
...
void f$die(const mixed& v) { f$exit(v); }
...
void print(const string& s) { print(s.c_str(), s.size()); }

void print(const string_buffer& sb) { print(sb.buffer(), sb.size()); }
...
void dbg_echo(const char* s) { dbg_echo(s, strlen(s)); }

void dbg_echo(const string& s) { dbg_echo(s.c_str(), s.size()); }

void dbg_echo(const string_buffer& sb) { dbg_echo(sb.buffer(), sb.size()); }

bool f$get_magic_quotes_gpc() { return false; }
...
string f$php_sapi_name() { return PhpScriptMutableGlobals::current().get_superglobals().v$d$PHP_SAPI; }
...
bool f$is_uploaded_file(const string& filename) { return (dl::query_num == uploaded_files_last_query_num && uploaded_files->get_value(filename) == 1); }
...
double f$get_net_time() { return get_net_time(); }

double f$get_script_time() { return get_script_time(); }

int64_t f$get_net_queries_count() { return get_net_queries_count(); }

int64_t f$get_engine_uptime() { return get_engine_uptime(); }

string f$get_engine_version() { return string(get_engine_version()); }

int64_t f$get_engine_workers_number() { return vk::singleton<WorkersControl>::get().get_total_workers_count(); }

string f$get_kphp_cluster_name() { return string{vk::singleton<ServerConfig>::get().get_cluster_name()}; }
...
void global_init_script_allocator() { dl::global_init_script_allocator(); }
...
void f$raise_sigsegv() { raise(SIGSEGV); }

void use_utf8() { is_utf8_enabled = true; }
...
bool f$extension_loaded(const string& /*extension*/) { return true; }
