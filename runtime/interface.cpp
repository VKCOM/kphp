...
static void init_superglobals_impl(const http_query_data &http_data, const rpc_query_data &rpc_data, const job_query_data &job_data) {
  ...
  if (http_data.uri) {
    if (http_data.get_len) {
      v$_SERVER.set_value(string("REQUEST_URI"), (vk::singleton<KphpRuntimeContext>::get().static_SB.clean() << uri_str << '?' << get_str).str());
    } else {
...
//          fprintf (stderr, "!%s!\n", p);
  ...
  v$_SERVER.set_value(string("SERVER_SIGNATURE"), (vk::singleton<KphpRuntimeContext>::get().static_SB.clean() << "Apache/2.2.9 (Debian) PHP/5.2.6-1<<lenny10 with Suhosin-Patch Server at "
