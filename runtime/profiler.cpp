...
public:
  ...
  T* get_top() const noexcept { return top_; }
...
FunctionStatsBase::FunctionStatsBase(const char* file, const char* function, size_t line, size_t level) noexcept
    : file_name(file), function_name(function), function_line(line), profiler_level(level) {}

void FunctionStatsBase::on_call_finish(const Stats& profiled_stats) noexcept { self_stats_ += profiled_stats; }
...
uint64_t FunctionStatsBase::calls_count() const noexcept { return self_stats_.calls; }
...
FunctionStatsNoLabel::FunctionStatsNoLabel(const char* file, const char* function, size_t line, size_t level) noexcept
    : FunctionStatsBase(file, function, line, level), TracingProfilerListNode<FunctionStatsNoLabel>(ProfilerContext::get().function_list.update_top(this)) {}
...
FunctionStatsWithLabel::FunctionStatsWithLabel(FunctionStatsNoLabel& no_label, vk::string_view label) noexcept
    : FunctionStatsBase(no_label.file_name, no_label.function_name, no_label.function_line, no_label.profiler_level), parent_stats_no_label(no_label) {
...
void ProfilerBase::forcibly_dump_log_on_finish(const char* function_name) noexcept {
  ...
  if (function_name) {
    ...
    std::replace_copy_if(suffix.begin(), suffix.begin() + suffix_size, suffix_buffer.begin(), [](char c) { return !isalnum(c); }, '_');
  }
...
void global_init_profiler() noexcept { ProfilerContext::get().global_init(); }
