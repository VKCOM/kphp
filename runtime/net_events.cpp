...
void update_precise_now() { kphp_precise_now = get_network_time(); }

double get_precise_now() { return kphp_precise_now; }
...
bool process_net_event(net_event_t* e) {
  ...
  std::visit(overloaded{[&](const net_events_data::rpc_answer& data) { process_rpc_answer(e->slot_id, data.result, data.result_len); },
                        [&](const net_events_data::rpc_error& data) { process_rpc_error(e->slot_id, data.error_code, data.error_message); },
                        [&](const net_events_data::job_worker_answer& data) { process_job_answer(e->slot_id, data.job_result); },
                        [&](database_drivers::Response* response) {
                          php_assert(e->slot_id == response->bound_request_id);
                          vk::singleton<database_drivers::Adaptor>::get().process_external_db_response_event(
...
kphp_event_timer* allocate_event_timer(double wakeup_time, int wakeup_callback_id, int wakeup_extra) {
  ...
  if (i == event_timers_max_heap_size) {
    event_timers_heap = static_cast<kphp_event_timer**>(
...
int wait_net(int timeout_ms) {
  ...
  if (some_expires && !finished_events) {
    php_warning("Have no finished events, but we must have them. begin_time = %.9lf, expire_event_time = %.9lf, timeout_ms = %d, now = %.9lf", begin_time,
                expire_event_time, timeout_ms, get_precise_now());
  }
