...
public:
  ...
  bool is_initialized() const noexcept { return global_manager_.is_initialized(); }

  const ConfdataPredefinedWildcards& get_predefined_wildcards() const noexcept { return global_manager_.get_predefined_wildcards(); }

  const ConfdataKeyBlacklist& get_key_blacklist() const noexcept { return global_manager_.get_key_blacklist(); }

private:
  ConfdataLocalManager() : global_manager_{ConfdataGlobalManager::get()} {};
...
bool f$is_confdata_loaded() noexcept { return ConfdataLocalManager::get().is_initialized(); }
