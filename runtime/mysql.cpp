...
string f$mysqli_error(const class_instance<C$mysqli>& db) { return db->error; }

int64_t f$mysqli_errno(const class_instance<C$mysqli>& db) { return db->errno_; }
...
bool f$mysqli_select_db(const class_instance<C$mysqli>& db __attribute__((unused)), const string& name __attribute__((unused))) { return true; }

static void reset_mysql_global_vars() { hard_reset_var(DB_Proxy); }

void init_mysql_lib() { reset_mysql_global_vars(); }

void free_mysql_lib() { reset_mysql_global_vars(); }
