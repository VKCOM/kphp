...
using runtime_injection::on_job_request_finish;
...
int64_t ProcessingJobs::finish_job_impl(int job_id, job_workers::FinishedJob *job_result, bool timeout) noexcept {
  ...
  if (job_result) {
    runtime_injection::invoke_callback(on_job_request_finish, job_id, false, now_timestamp - ready_job.send_timestamp, fork_id);
    ...
    runtime_injection::invoke_callback(on_job_request_finish, job_id, error.get()->error_code, now_timestamp - ready_job.send_timestamp, fork_id);
...
const JobRequestInfo &ProcessingJobs::get_job_request_info(int job_id) noexcept {
  return processing_[job_id];
}
