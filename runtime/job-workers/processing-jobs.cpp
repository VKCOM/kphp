...
void ProcessingJobs::start_job_processing(int job_id, JobRequestInfo&& job_request_info) noexcept { processing_[job_id] = std::move(job_request_info); }

int64_t ProcessingJobs::finish_job_on_answer(int job_id, job_workers::FinishedJob* job_result) noexcept { return finish_job_impl(job_id, job_result, false); }

int64_t ProcessingJobs::finish_job_on_timeout(int job_id) noexcept { return finish_job_impl(job_id, nullptr, true); }
