...
struct C$KphpJobWorkerResponseError : public refcountable_polymorphic_php_classes<C$KphpJobWorkerResponse> {
  ...
  const char* get_class() const noexcept { return R"(KphpJobWorkerResponseError)"; }

  int get_hash() const noexcept { return static_cast<int32_t>(vk::std_hash(vk::string_view(C$KphpJobWorkerResponseError::get_class()))); }
  ...
  void accept(InstanceReferencesCountingVisitor& visitor) noexcept { return generic_accept(visitor); }

  void accept(InstanceDeepCopyVisitor& visitor) noexcept { return generic_accept(visitor); }

  void accept(InstanceDeepDestroyVisitor& visitor) noexcept { return generic_accept(visitor); }

  void accept(ToArrayVisitor& visitor) noexcept { return generic_accept(visitor); }

  void accept(CommonMemoryEstimateVisitor& visitor) noexcept;

  size_t virtual_builtin_sizeof() const noexcept { return sizeof(*this); }

  C$KphpJobWorkerResponseError* virtual_builtin_clone() const noexcept { return new C$KphpJobWorkerResponseError{*this}; }
};
