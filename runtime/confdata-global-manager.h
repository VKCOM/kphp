...
public:
  ...
  const auto& get_confdata() const noexcept { return *confdata_storage_; }
...
public:
  ...
  ConfdataSample& get_current() noexcept { return confdata_samples_.get_current_resource(); }

  const ConfdataSample* acquire_current_sample() noexcept { return confdata_samples_.acquire_current_resource(); }

  void release_sample(const ConfdataSample* sample) noexcept { return confdata_samples_.release_resource(sample); }

  bool can_next_be_updated() noexcept { return confdata_samples_.is_next_resource_unused(); }
  ...
  void clear_unused_samples() noexcept { return confdata_samples_.clear_dirty_unused_resources_in_sequence(); }

  memory_resource::unsynchronized_pool_resource& get_resource() noexcept { return resource_; }

  bool is_initialized() const noexcept { return resource_.memory_begin(); }

  const ConfdataPredefinedWildcards& get_predefined_wildcards() const noexcept { return predefined_wildcards_; }

  const ConfdataKeyBlacklist& get_key_blacklist() const noexcept { return key_blacklist_; }
