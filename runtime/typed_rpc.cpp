...
public:
  typed_rpc_tl_query_result_one_resumable(const class_instance<RpcTlQuery>& query, const RpcErrorFactory& error_factory)
      : query_(query), error_factory_(error_factory) {}
...
array<class_instance<C$VK$TL$RpcResponse>> sort_rpc_results(const array<class_instance<C$VK$TL$RpcResponse>>& unsorted_results, const array<int64_t>& query_ids,
  ...
  for (auto it = query_ids.begin(); it != query_ids.end(); ++it) {
    ...
    if (!unsorted_results.isset(query_id)) {
      string err = query_id <= 0 ? string("Very wrong query_id ") : string("No answer received or duplicate/wrong query_id ");
...
public:
  explicit typed_rpc_tl_query_result_resumable(const array<int64_t>& query_ids, const RpcErrorFactory& error_factory)
      : query_ids_(query_ids), unsorted_results_(array_size(query_ids_.count(), false)), error_factory_(error_factory) {}
...
private:
  ...
  bool run() {
    ...
    if (query_ids_.count() == 1) {
      ...
      TRY_WAIT(rpc_tl_query_result_resumable_label_0, unsorted_results_[query_id_.val()], class_instance<C$VK$TL$RpcResponse>);
    } else {
