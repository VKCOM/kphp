...
public:
  ...
  size_t get_max_wildcards_for_element() const noexcept { return max_wildcards_for_element_; }

  bool has_wildcard(vk::string_view wildcard) const noexcept { return wildcards_.find(wildcard) != wildcards_.end(); }
  ...
  bool has_wildcard_for_key(vk::string_view first_key) const noexcept { return !make_predefined_wildcard_len_range_by_key(first_key).empty(); }
