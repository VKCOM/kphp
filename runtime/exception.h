...
struct C$Throwable : public refcountable_polymorphic_php_classes_virt<> {
  virtual const char* get_class() const noexcept { return "Throwable"; }

  virtual int32_t get_hash() const noexcept { return static_cast<int32_t>(vk::std_hash(vk::string_view(get_class()))); }
  ...
  virtual void accept(CommonMemoryEstimateVisitor& visitor) noexcept { generic_accept(visitor); }

  virtual void accept(InstanceDeepCopyVisitor& visitor) noexcept { generic_accept(visitor); }

  virtual void accept(InstanceDeepDestroyVisitor& visitor) noexcept { generic_accept(visitor); }

  virtual void accept(InstanceReferencesCountingVisitor& visitor) noexcept { generic_accept(visitor); }
...
struct C$Exception : public C$Throwable {
  ...
  C$Exception* virtual_builtin_clone() const noexcept { return new C$Exception{*this}; }

  size_t virtual_builtin_sizeof() const noexcept { return sizeof(*this); }
...
struct C$Error : public C$Throwable {
  ...
  C$Error* virtual_builtin_clone() const noexcept { return new C$Error{*this}; }
...
inline class_instance<T> make_throwable(const string& file, int64_t line, int64_t code, const string& desc) noexcept {
  static_assert(std::is_base_of_v<C$Throwable, T>, "Template argument must be a subtype of C$Throwable");
