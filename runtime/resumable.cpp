...
static bool in_main_thread() noexcept { return runned_resumable_id == 0; }
...
void Resumable::update_output() { output_ = in_main_thread() ? nullptr : get_storage(runned_resumable_id); }
...
inline bool is_forked_resumable_id(int64_t resumable_id) { return first_forked_resumable_id <= resumable_id && resumable_id < current_forked_resumable_id; }
...
inline bool is_started_resumable_id(int64_t resumable_id) { return first_started_resumable_id <= resumable_id && resumable_id < current_started_resumable_id; }
...
inline bool is_wait_queue_id(int64_t queue_id) { return 0 < queue_id && queue_id <= wait_next_queue_id && wait_queues[queue_id - 1].resumable_id >= 0; }
...
Storage* get_started_storage(int64_t resumable_id) noexcept { return &get_started_resumable_info(resumable_id)->output; }

} // namespace

Storage* get_forked_storage(int64_t resumable_id) { return &get_forked_resumable_info(resumable_id)->output; }
...
int64_t register_forked_resumable(Resumable* resumable) {
  if (current_forked_resumable_id == first_array_forked_resumable_id + forked_resumables_size) {
    ...
    while (first_needed_id < forked_resumables_size && forked_resumables[first_needed_id].queue_id == -1 &&
    ...
    if (first_needed_id > forked_resumables_size / 2) {
      ...
      forked_resumables = static_cast<forked_resumable_info*>(dl::reallocate(forked_resumables, sizeof(forked_resumable_info) * 2 * forked_resumables_size,
                                                                             sizeof(forked_resumable_info) * forked_resumables_size));
...
static int64_t register_started_resumable(Resumable* resumable) noexcept {
  ...
  if (first_free_started_resumable_id) {
    ...
    if (current_started_resumable_id == first_started_resumable_id + started_resumables_size) {
      started_resumables = static_cast<started_resumable_info*>(dl::reallocate(started_resumables, sizeof(started_resumable_info) * 2 * started_resumables_size,
                                                                               sizeof(started_resumable_info) * started_resumables_size));
  ...
  if (!in_main_thread()) {
    auto* parent = is_started_resumable_id(runned_resumable_id) ? static_cast<resumable_info*>(get_started_resumable_info(runned_resumable_id))
...
Resumable* get_forked_resumable(int64_t resumable_id) { return get_forked_resumable_info(resumable_id)->continuation; }

Resumable* get_started_resumable(int64_t resumable_id) { return get_started_resumable_info(resumable_id)->continuation; }
...
static void add_resumable_to_queue(int64_t resumable_id, forked_resumable_info* resumable) {
  ...
  tvkprintf(resumable, 2, "Push resumable %" PRIi64 " to queue %" PRIi64 "(%" PRIi64 ", %" PRIi64 ", %" PRIi32 ") at %.6lf\n", resumable_id,
            resumable->queue_id, q->resumable_id, q->first_finished_function, q->left_functions, (update_precise_now(), get_precise_now()));
...
static void unregister_started_resumable(int64_t resumable_id) noexcept {
  started_resumable_info* res = get_started_resumable_info(resumable_id);
  if (res->parent_id > 0) {
    auto* parent = is_started_resumable_id(res->parent_id) ? static_cast<resumable_info*>(get_started_resumable_info(res->parent_id))
...
static bool resumable_has_finished() { return finished_resumables_count > 0 || yielded_resumables_l != yielded_resumables_r; }
...
static void yielded_resumables_push(int64_t id) {
  ...
  if (yielded_resumables_r == yielded_resumables_l) {
    yielded_resumables =
        static_cast<int64_t*>(dl::reallocate(yielded_resumables, sizeof(int64_t) * 2 * yielded_resumables_size, sizeof(int64_t) * yielded_resumables_size));
...
static void resumable_add_finished(int64_t resumable_id) {
  ...
  if (finished_resumables_count >= finished_resumables_size) {
    ...
    finished_resumables =
        static_cast<int64_t*>(dl::reallocate(finished_resumables, sizeof(int64_t) * 2 * finished_resumables_size, sizeof(int64_t) * finished_resumables_size));
...
static void debug_print_resumables() noexcept {
  ...
  for (int64_t i = first_started_resumable_id; i < current_started_resumable_id; i++) {
    ...
    fprintf(stderr, "started id = %" PRIi64 ", parent = %" PRIi64 ", fork = %" PRIi64 ", son = %" PRIi64 ", continuation = %s, name = %s\n", i, info->parent_id,
            info->fork_id, info->son, info->continuation ? typeid(*info->continuation).name() : "(none)", info->name ? info->name : "()");
  }
...
public:
  ...
  explicit wait_resumable(int64_t child_id) noexcept : child_id_(child_id) {}

  void set_timer(double timeout, int64_t wakeup_extra) noexcept { ResumableWithTimer::set_timer(timeout, wait_timeout_wakeup_id, wakeup_extra); }
...
public:
  explicit wait_concurrently_resumable(int64_t child_id) noexcept : child_id_(child_id) {}
...
bool wait_without_result(int64_t resumable_id, double timeout) {
  ...
  if (resumable->queue_id > 0) {
    ...
    php_warning("Resumable is already waited by other thread: waiter=%d(%s), fork=%d(%s), running_resumable_id=%d", waiter_id,
                waiter_resumable ? typeid(*waiter_resumable).name() : "null", fork_id, waiter_fork ? typeid(*waiter_fork).name() : "null",
...
int64_t f$wait_queue_create(const mixed& resumable_ids) { return f$wait_queue_push(-1, resumable_ids); }
...
public:
  ...
  explicit wait_queue_resumable(int64_t queue_id) noexcept : queue_id_(queue_id) {}

  void set_timer(double timeout, int64_t wakeup_extra) noexcept { ResumableWithTimer::set_timer(timeout, wait_queue_timeout_wakeup_id, wakeup_extra); }
...
Optional<int64_t> f$wait_queue_next(int64_t queue_id, double timeout) {
  ...
  if (timeout == 0.0) {
    ...
    return q->first_finished_function == -2 ? Optional<int64_t>{false} : Optional<int64_t>{-q->first_finished_function};
  }
