...
const char *php_uncaught_exception_error(const class_instance<C$Throwable> &ex) noexcept {
  ...
  if (is_demangled_stacktrace_logs_enabled) {
    vk::singleton<JsonLogger>::get().write_log_with_demangled_backtrace(dl_pstr("Unhandled %s from %s:%" PRIi64 "; Error %" PRIi64 "; Message: %s", ex->get_class(), src_file, ex->$line, ex->$code, message),
                                                                        E_ERROR, current_time, ex->raw_trace.get_const_vector_pointer(), ex->raw_trace.count(), true);
  } else {
    vk::singleton<JsonLogger>::get().write_log(dl_pstr("Unhandled %s from %s:%" PRIi64 "; Error %" PRIi64 "; Message: %s", ex->get_class(), src_file, ex->$line, ex->$code, message),
                                               E_ERROR, current_time, ex->raw_trace.get_const_vector_pointer(), ex->raw_trace.count(), true);
  }
