...
const char* last_rpc_error_message_get() { return last_rpc_error; }

int32_t last_rpc_error_code_get() { return last_rpc_error_code; }
...
bool f$rpc_parse(bool new_rpc_data) { return f$rpc_parse(mixed{new_rpc_data}); }
...
int32_t rpc_get_pos() { return static_cast<int32_t>(rpc_data - rpc_data_begin); }
...
int64_t f$fetch_int() { return rpc_fetch_int(); }

int64_t f$fetch_lookup_int() { return rpc_lookup_int(); }
...
bool f$fetch_eof() { return rpc_data_len == 0; }
...
C$RpcConnection::C$RpcConnection(int32_t host_num, int32_t port, int32_t timeout_ms, int32_t actor_id, int32_t connect_timeout, int32_t reconnect_timeout)
    : host_num(host_num), port(port), timeout_ms(timeout_ms), actor_id(actor_id), connect_timeout(connect_timeout), reconnect_timeout(reconnect_timeout) {}
...
void f$store_gzip_pack_threshold(int64_t pack_threshold_bytes) { rpc_pack_threshold = pack_threshold_bytes; }

void f$store_start_gzip_pack() { rpc_pack_from = data_buf.size(); }
...
bool f$store_header(const mixed& cluster_id, int64_t flags) { return store_header(store_parse_number<long long>(cluster_id), flags); }
...
bool f$store_error(int64_t error_code, const string& error_text) { return store_error(error_code, error_text.c_str(), (int)error_text.size()); }
...
bool store_int(int32_t v) { return store_raw(v); }
...
bool store_long(long long v) { return store_raw(v); }

bool f$store_double(double v) { return store_raw(v); }

bool f$store_float(double v) { return store_raw(static_cast<float>(v)); }
...
bool f$store_string(const string& v) { return store_string(v.c_str(), (int)v.size()); }
...
bool f$store_finish() { return rpc_store(false); }
...
public:
  explicit rpc_resumable(int request_id) : request_id(request_id) {}

  bool is_internal_resumable() const noexcept final { return true; }
};

static array<double> rpc_request_need_timer;

static void process_rpc_timeout(int request_id, bool fake_timeout_for_no_result) {
  process_rpc_error(request_id, TL_ERROR_TIMEOUT_IN_RPC_CLIENT, "Timeout in KPHP runtime", fake_timeout_for_no_result);
}

static void process_rpc_timeout(kphp_event_timer* timer) { return process_rpc_timeout(timer->wakeup_extra, false); }
...
int64_t rpc_send_impl(const class_instance<C$RpcConnection>& conn, double timeout, rpc_request_extra_info_t& req_extra_info, bool collect_resp_extra_info,
  ...
  if (opt_new_wrapper.has_value()) {
    ...
    std::memcpy(request_buf + sizeof(RpcHeaders) + new_wrapper_size, data_buf.c_str() + sizeof(RpcHeaders) + cur_wrapper_size,
  ...
  if (dl::query_num != rpc_requests_last_query_num) {
    ...
    rpc_first_request_id = q_id;
    rpc_first_array_request_id = q_id;
    rpc_next_request_id = q_id + 1;
    rpc_first_unfinished_request_id = q_id;
  ...
  rpc_request* cur = get_rpc_request(q_id);
  ...
  if (ignore_answer) {
    ...
    rpc_request_need_timer.set_value(q_id, timeout);
...
void process_rpc_answer(int32_t request_id, char* result, int32_t result_len) {
  ...
  if (kphp_tracing::is_turned_on()) {
    kphp_tracing::on_rpc_query_finish(request_id, result_len);
  }
...
void process_rpc_error(int32_t request_id, int32_t error_code, const char* error_message, bool is_fake_error) {
  rpc_request* request = get_rpc_request(request_id);
...
public:
  rpc_get_resumable(int64_t resumable_id, double timeout) : resumable_id(resumable_id), timeout(timeout), ready(false) {}
};
...
public:
  rpc_get_and_parse_resumable(int64_t resumable_id, double timeout) : resumable_id(resumable_id), timeout(timeout), ready(false) {}
};
...
bool rpc_get_and_parse(int64_t request_id, double timeout) { return start_resumable<bool>(new rpc_get_and_parse_resumable(request_id, timeout)); }

int64_t f$query_x2(int64_t x) { return query_x2(static_cast<int32_t>(x)); }
...
bool store_long(const mixed& v) { return store_long(store_parse_number<long long>(v)); }

bool f$store_long(int64_t v) { return store_long(v); }
...
void tl_parse_end() { TRY_CALL_VOID(void, (f$fetch_end())); }

int tl_parse_save_pos() { return rpc_get_pos(); }

bool tl_parse_restore_pos(int pos) { return rpc_set_pos(pos); }
...
array<mixed> tl_fetch_error(const char* error, int error_code) { return tl_fetch_error(string(error), error_code); }
...
public:
  rpc_tl_query_result_one_resumable(int64_t query_id, class_instance<RpcTlQuery>&& rpc_query) : query_id(query_id), rpc_query(std::move(rpc_query)) {}
};
      ...
      if (!tl_objects_unsorted.isset(query_id)) {
        if (query_id <= 0) {
          tl_objects[it.get_key()] =
              tl_fetch_error((kphp_runtime_context.static_SB.clean() << "Very wrong query_id " << query_id).str(), TL_ERROR_WRONG_QUERY_ID);
        } else {
          tl_objects[it.get_key()] = tl_fetch_error(
              (kphp_runtime_context.static_SB.clean() << "No answer received or duplicate/wrong query_id " << query_id).str(), TL_ERROR_WRONG_QUERY_ID);
        }
      } else {
...
public:
  explicit rpc_tl_query_result_resumable(const array<int64_t>& query_ids)
      : query_ids(query_ids), tl_objects_unsorted(array_size(query_ids.count(), false)), queue_id(0), query_id(0) {}
};
...
int64_t f$rpc_queue_create() { return f$wait_queue_create(); }

int64_t f$rpc_queue_create(const mixed& request_ids) { return f$wait_queue_create(request_ids); }

int64_t f$rpc_queue_push(int64_t queue_id, const mixed& request_ids) { return f$wait_queue_push(queue_id, request_ids); }

bool f$rpc_queue_empty(int64_t queue_id) { return f$wait_queue_empty(queue_id); }

Optional<int64_t> f$rpc_queue_next(int64_t queue_id, double timeout) { return f$wait_queue_next(queue_id, timeout); }

Optional<int64_t> f$rpc_queue_next_synchronously(int64_t queue_id) { return f$wait_queue_next_synchronously(queue_id); }

bool f$rpc_wait(int64_t request_id) { return wait_without_result(request_id); }

bool f$rpc_wait_concurrently(int64_t request_id) { return f$wait_concurrently(request_id); }
