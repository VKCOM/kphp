...
public:
  explicit TracingProfilerListNode(T* next = nullptr) : next_(next) {}

  T* get_next() const noexcept { return next_; }
...
public:
  ...
  const auto& get_labels() const noexcept { return labels_; }

  void write_function_name_with_label(FILE* out) const noexcept final { fprintf(out, "%s", function_name); }
...
public:
  ...
  const char* get_label() const noexcept { return label_buffer_.data(); }

  static constexpr size_t label_max_len() noexcept { return label_max_len_; }

  void write_function_name_with_label(FILE* out) const noexcept final { fprintf(out, "%s (%s)", function_name, label_buffer_.data()); }

  FunctionStatsWithLabel& get_stats_with_label(vk::string_view label) noexcept final { return parent_stats_no_label.get_stats_with_label(label); }
