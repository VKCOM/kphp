...
//  fprintf (stderr, "command_net_write [ptr=%p] [len = %d] [data = %p]\n", base_command, command->len, data);
  ...
  assert (command->data != NULL);
  if (data == NULL) { //send to /dev/null
    vkprintf (3, "failed to send rpc request %d\n", slot_id);
    on_net_event (create_rpc_error_event (slot_id, TL_ERROR_NO_CONNECTIONS, "Failed to send query, timeout expired", NULL));
  } else {
    struct connection *d = (struct connection *)data;
    //assert (d->status == conn_ready);
...
void php_worker_run_query_x2 (php_worker *worker __attribute__((unused)), php_query_x2_t *query) {//TODO: this unused generated by script, recheck it.
...
void php_worker_run_query_connect (php_worker *worker __attribute__((unused)), php_query_connect_t *query) {//TODO: this unused generated by script, recheck it.
  php_script_query_readed (php_script);
...
void php_worker_run_rpc_send_query (net_query_t *query) {
  ...
  if (conn != NULL) {
    ...
    int new_conn_cnt = create_new_connections (target);
    if (new_conn_cnt <= 0 && get_target_connection (target, 1) == NULL) {
      on_net_event (create_rpc_error_event (slot_id, TL_ERROR_NO_CONNECTIONS, "Failed to establish connection [probably reconnect timeout is not expired]", NULL));
      return;
    }
...
void php_worker_run_net_queue (php_worker *worker __attribute__((unused))) {//TODO: this unused generated by script, recheck it.
  net_query_t *query;
  while ((query = pop_net_query()) != NULL) {
...
void hts_my_func_finish (struct connection *c __attribute__((unused))) {//TODO: this unused generated by script, recheck it.
/*  c->status = conn_expect_query;
...
int hts_func_close (struct connection *c, int who __attribute__((unused))) {//TODO: this unused generated by script, recheck it.
  struct hts_data *D = HTS_DATA(c);
...
int rpcx_func_close (struct connection *c, int who __attribute__((unused))) {//TODO: this unused generated by script, recheck it.
  struct rpcs_data *D = RPCS_DATA(c);
...
int stop_forwarding_response (struct connection *c __attribute__((unused))) {//TODO: this unused generated by script, recheck it.
  ...
  switch (op) {
    ...
    case RPC_REQ_RESULT:
      ...
      if (op == RPC_REQ_ERROR) {
        ...
        event_status = create_rpc_error_event (id, -1, "unknown error", NULL);
        break;
      }
...
static void sigint_immediate_handler (const int sig __attribute__((unused))) {//TODO: this unused generated by script, recheck it.
...
static void sigterm_immediate_handler (const int sig __attribute__((unused))) {//TODO: this unused generated by script, recheck it.
...
void write_full_stats_to_pipe (void) {
...
void write_immediate_stats_to_pipe (void) {
...
static void sigstats_handler (int signum __attribute__((unused)), siginfo_t *info, void *data __attribute__((unused))) {//TODO: this unused generated by script, recheck it.
