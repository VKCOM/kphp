...
typename array<T>::array_inner *array<T>::array_inner::create(int64_t new_int_size, bool is_vector) {
  ...
  if (is_vector) {
    auto p = reinterpret_cast<array_inner *>(CoreAllocator::current().alloc_script_memory(mem_size));
  ...
  array_inner *p = shift_pointer_to_array_inner(CoreAllocator::current().alloc0_script_memory(mem_size));
  ...
  if (ref_cnt < ExtraRefCnt::for_global_const) {
    ...
    if (ref_cnt <= -1) {
      if (is_vector()) {
        ...
        CoreAllocator::current().free_script_memory((void *)this, sizeof_vector(buf_size));
        return;
      }
      ...
      CoreAllocator::current().free_script_memory(shifted_this, sizeof_map(buf_size));
    }
  }
}
...
void array<T>::mutate_to_size(int64_t int_size) {
  ...
  p = static_cast<array_inner *>(CoreAllocator::current().realloc_script_memory(p, p->sizeof_vector(new_int_buff_size), p->sizeof_vector(p->buf_size)));
  ...
  if (is_vector()) {
    if (other.is_vector()) {
      ...
      if (p->ref_cnt > 0) {
        ...
        p = (array_inner *)CoreAllocator::current().realloc_script_memory((void *)p, p->sizeof_vector(new_size), p->sizeof_vector(p->buf_size));
        p->buf_size = new_size;
      }
...
void array<T>::sort(const T1 &compare, bool renumber) {
  ...
  array_bucket **arTmp = (array_bucket **)CoreAllocator::current().alloc_script_memory(n * sizeof(array_bucket * ));
  uint32_t i = 0;
  for (array_bucket *it = p->begin(); it != p->end(); it = p->next(it)) {
    arTmp[i++] = it;
  }
  ...
  CoreAllocator::current().free_script_memory(arTmp, n * sizeof(array_bucket * ));
}
