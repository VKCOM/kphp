...
void write_rpc_server_functions(CodeGenerator& W) {
  ...
  for (const auto& f : kphp_functions) {
    ...
    W << get_php_runtime_type(f, true) << " request;" << NL << "request.alloc();" << NL
      << "CurrentTlQuery::get().set_current_tl_function(f$VK$TL$RpcFunction$$getTLFunctionName(request));" << NL
      << "auto custom_fetcher = f$VK$TL$RpcFunction$$typedFetch(request);" << NL << "CHECK_EXCEPTION(" << NL
      << "  php_warning(\"Exception when calling typedFetch for function magic: 0x%08x tl_version: %d\", function_magic, tl_version);" << NL << "  return {};"
      << NL << ")" << NL << "if (custom_fetcher.is_null())" << BEGIN << "if (tl_version == 2)" << BEGIN
      ...
      << "return {};" << NL << END << NL << "(void)f$fetch_int();" << NL << "CurrentRpcServerQuery::get().save(" << cpp_tl_struct_name("f_", f->name)
      << "::rpc_server_typed_fetch(request.get()));" << NL << END << "else" << BEGIN
      << "CurrentRpcServerQuery::get().save(make_tl_func_base_simple_wrapper(std::move(custom_fetcher)));" << NL << END << NL << "return request;" << NL << END
      << NL;
  }
  W << "default: " << BEGIN
    << "php_warning(\"Unexpected function magic on fetching request in rpc server: 0x%08x tl_version: %d\", function_magic, tl_version);" << NL << "return {};"
    << NL << END << NL;
  W << END << NL;
  W << END << NL;
  W << CloseFile();
