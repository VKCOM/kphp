...
void TlFunctionDef::compile(CodeGenerator &W) const {
  ...
  if (needs_typed_fetch_store) {
    ...
    FunctionSignatureGenerator(W) << "class_instance<" << G->env().get_tl_classname_prefix() << "RpcFunctionReturnResult> " << struct_name << "::typed_fetch() " << BEGIN;
  ...
  if (f->is_kphp_rpc_server_function() && needs_typed_fetch_store) {
    ...
    W << "return std::move(tl_func_state);" << NL;
    W << END << NL << NL;
    FunctionSignatureGenerator(W) << "void " << struct_name << "::rpc_server_typed_store(const class_instance<" << G->env().get_tl_classname_prefix()
