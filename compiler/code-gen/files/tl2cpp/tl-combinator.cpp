...
void CombinatorStore::gen_before_args_processing(CodeGenerator &W) const {
  ...
  if (combinator->is_function()) {
    ...
    W << fmt_format("get_component_context()->rpc_component_context.current_query.set_last_stored_tl_function_magic({:#010x});", static_cast<unsigned int>(combinator->id)) << NL;
  }
}
  ...
  if (arg->is_fields_mask_optional()) {
    ...
    if (typed_mode) {
      ...
      if (!value_check.empty()) {
        ...
        W
          << fmt_format(R"(get_component_context()->rpc_component_context.current_query.raise_storing_error("Optional field %s of %s is not set, but corresponding fields mask bit is set", "{}", "{}");)",
  ...
  if (arg->is_forwarded_function()) {
    ...
    if (!typed_mode) {
      ...
      W << "if (tl_object->$" << arg->name << ".is_null()) " << BEGIN
        << R"(get_component_context()->rpc_component_context.current_query.raise_storing_error("Field \")" << arg->name << R"(\" not found in tl object");)" << NL
        << "return {};" << NL
        << END << NL;
