  ...
  if (arg->is_fields_mask_optional()) {
    ...
    if (typed_mode) {
      ...
      if (!value_check.empty()) {
        ...
        W << fmt_format(
                 R"(CurrentTlQuery::get().raise_storing_error("Optional field %s of %s is not set, but corresponding fields mask bit is set", "{}", "{}");)",
  ...
  if (arg->is_forwarded_function()) {
    ...
    if (!typed_mode) {
      ...
      W << "auto _cur_arg = "
        ...
        << NL;
      W << "mixed target_f_name_mixed = " << fmt_format("tl_arr_get(_cur_arg, {}, 0, {}L)", tl2cpp::register_tl_const_str("_"), tl2cpp::hash_tl_const_str("_"))
        << ";" << NL;
      W << "if (!target_f_name_mixed.is_string()) " << BEGIN << R"(CurrentTlQuery::get().raise_storing_error("Field \"_\" of \")" << arg->name
        << R"(\" array is not string");)" << NL << "return {};" << NL << END << NL;
      ...
      W << fmt_format("if (!{}tl_storers_ht.has_key(target_f_name)) ", k2_tl_storers_prefix) << BEGIN
        << "CurrentTlQuery::get().raise_storing_error(\"Function %s not found in tl-scheme\", target_f_name.c_str());" << NL << "return {};" << NL << END << NL;
      ...
      W << "if (tl_object->$" << arg->name << ".is_null()) " << BEGIN << R"(CurrentTlQuery::get().raise_storing_error("Field \")" << arg->name
