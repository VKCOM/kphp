...
void CombinatorStore::gen_arg_processing(CodeGenerator &W, const std::unique_ptr<vk::tlo_parsing::arg> &arg) const {
  ...
  if (arg->is_forwarded_function()) {
    ...
    if (!typed_mode) {
      ...
      W << "auto _cur_arg = "
        << fmt_format("TRY_CALL(mixed, std::nullptr_t, tl_arr_get(tl_object, {}, {}, {}L))", tl2cpp::register_tl_const_str(arg->name), arg->idx,
                      tl2cpp::hash_tl_const_str(arg->name))
        << ";" << NL;
      W << "string target_f_name = "
        << fmt_format("TRY_CALL(mixed, std::nullptr_t, tl_arr_get(_cur_arg, {}, 0, {}L)).to_string()", tl2cpp::register_tl_const_str("_"),
