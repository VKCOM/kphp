...
struct RunInterruptedFunction {
  ...
  void compile(CodeGenerator& W) const {
    std::string await_prefix = function->is_interruptible ? "CO_AWAIT_TASK_ON_STACK(" : "";
    std::string await_suffix = function->is_interruptible ? ")" : "";
...
void ComponentInfoFile::compile(CodeGenerator& W) const {
  ...
  if (!G->settings().php_code_commit_hash.get().empty()) {
    ...
    for (size_t i{1}; i != 40 && i != G->settings().php_code_commit_hash.get().size(); ++i) {
      php_code_commit_hash.append(", ").append(std::to_string(static_cast<uint8_t>(G->settings().php_code_commit_hash.get()[i])));
    }
  }
