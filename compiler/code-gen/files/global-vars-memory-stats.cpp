...
void GlobalVarsMemoryStats::compile(CodeGenerator& W) const {
  ...
  W << OpenFile("globals_memory_stats.cpp", "", false) << ExternInclude(G->settings().runtime_headers.get()) << OpenNamespace();
  ...
  FunctionSignatureGenerator(W) << "array<int64_t> " << getter_name_ << "(int64_t lower_bound, " << PhpMutableGlobalsConstRefArgument() << ")" << BEGIN
                                << "array<int64_t> result;" << NL << "result.reserve(" << total_count << ", false);" << NL << NL;

  for (int part_id = 0; part_id < parts_cnt; ++part_id) {
    const std::string func_name_i = getter_name_ + std::to_string(part_id);
    // function declaration
    FunctionSignatureGenerator(W) << "void " << func_name_i << "(int64_t lower_bound, array<int64_t> &result, " << PhpMutableGlobalsConstRefArgument() << ")"
                                  << SemicolonAndNL();
    // function call
...
void GlobalVarsMemoryStats::compile_getter_part(CodeGenerator& W, int part_id, const std::vector<VarPtr>& global_vars, int offset, int count) {
  ...
  FunctionSignatureGenerator(W) << "void " << getter_name_ << part_id << "(int64_t lower_bound, array<int64_t> &result, " << PhpMutableGlobalsConstRefArgument()
                                << ")" << BEGIN;
  ...
  for (int i = 0; i < count; ++i) {
    VarPtr global_var = global_vars[offset + i];
    W << "// " << var_names[i] << NL << "estimation = f$estimate_memory_usage(" << GlobalVarInPhpGlobals(global_var) << ");" << NL
      << "if (estimation > lower_bound) " << BEGIN << "result.set_value(get_raw_string(" << var_name_shifts[i] << "), estimation);" << NL << END << NL;
  }
