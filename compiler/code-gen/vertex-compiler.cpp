...
bool contains_interruptible_call(VertexPtr vertex) {
  if (auto func_call = vertex.try_as<op_func_call>()) {
    if (func_call->func_id && func_call->func_id->is_interruptible) {
      return true;
    }
  }
...
void compile_null_coalesce(VertexAdaptor<op_null_coalesce> root, CodeGenerator &W) {
  ...
  if (vk::any_of_equal(rhs->type(), op_var, op_int_const, op_float_const, op_false, op_null)) {
    ...
    const auto saved_interruptible_flag = std::exchange(context.interruptible_flag, interruptible_call);
...
void compile_func_call(VertexAdaptor<op_func_call> root, CodeGenerator &W, func_call_mode mode = func_call_mode::simple) {
    ...
    if (mode == func_call_mode::fork_call) {
      if (G->is_output_mode_k2()) {
        if (func->is_interruptible) {
          ...
          W << "kphp::forks::start(kphp::coro::lift_sync(" << FunctionName(func);
          if (!root->args().empty()) {
            W << ", ";
          }
        }
  ...
  if (mode == func_call_mode::fork_call && G->is_output_mode_k2()) {
    if (func->is_interruptible) {
      W << ")";
    } else {
      W << "))";
    }
  } else if (func->is_interruptible) {
    W << ")";
  }
}
...
void compile_callback_of_builtin(VertexAdaptor<op_callback_of_builtin> root, CodeGenerator &W) {
  ...
  if (k2_async_callback) { // add explicit return type to make this lambda async
    W << "-> decltype(" << FunctionName(root->func_id) << "(";
    for (auto idx = 1; idx <= root->size(); ++idx) {
      W << "captured" << idx << ", ";
    }
