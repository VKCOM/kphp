...
static void hard_link_or_copy_impl(const std::string& from, const std::string& to, bool replace, bool allow_copy) noexcept {
  ...
  if (errno == EEXIST) {
    if (replace) {
      kphp_error_return(!unlink(to.c_str()), fmt_format("Can't remove file '{}': {}", to, strerror(errno)));
  ...
  if (vk::any_of_equal(errno, EXDEV, EPERM) && allow_copy) {
    ...
    kphp_error_return(!stat(from.c_str(), &file_stat), fmt_format("Can't get file stat '{}': {}", from, strerror(errno)));

    std::string tmp_file = to + ".XXXXXX";
    int tmp_fd = mkstemp(&tmp_file[0]);
    kphp_error_return(tmp_fd != -1, fmt_format("Can't create tmp file '{}': {}", tmp_file, strerror(errno)));
    kphp_error_return(fchmod(tmp_fd, file_stat.st_mode) != -1, fmt_format("Can't change permissions of tmp file '{}': {}", tmp_file, strerror(errno)));
