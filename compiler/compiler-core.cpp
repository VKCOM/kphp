...
SrcFilePtr CompilerCore::register_file(const std::string &file_name, LibPtr owner_lib, bool builtin) {
  ...
  if (!node->data) {
    AutoLocker<Lockable *> locker(node);
    if (!node->data) {
      stats.ht_total_files.fetch_add(1, std::memory_order_relaxed);
...
SrcDirPtr CompilerCore::register_dir(vk::string_view full_dir_name) {
  ...
  stats.ht_total_dirs.fetch_add(1, std::memory_order_relaxed);
...
LibPtr CompilerCore::register_lib(LibPtr lib) {
  stats.ht_total_libs.fetch_add(1, std::memory_order_relaxed);
...
ModulitePtr CompilerCore::register_modulite(ModulitePtr modulite) {
  stats.ht_total_modulites.fetch_add(1, std::memory_order_relaxed);
...
ComposerJsonPtr CompilerCore::register_composer_json(ComposerJsonPtr composer_json) {
  stats.ht_total_composer_jsons.fetch_add(1, std::memory_order_relaxed);
...
bool CompilerCore::register_define(DefinePtr def_id) {
  ...
  stats.ht_total_defines.fetch_add(1, std::memory_order_relaxed);
...
void CompilerCore::update_hash_tables_stats() {
  ...
  stats.ht_max_files = file_ht.get_size();
  stats.ht_max_dirs = dirs_ht.get_size();
  stats.ht_max_functions = functions_ht.get_size();
  stats.ht_max_classes = classes_ht.get_size();
  stats.ht_max_defines = defines_ht.get_size();
  stats.ht_max_constants = constants_ht.get_size();
  stats.ht_max_globals = globals_ht.get_size();
  stats.ht_max_libs = libs_ht.get_size();
  stats.ht_max_modulites = modulites_ht.get_size();
  stats.ht_max_composer_jsons = composer_json_ht.get_size();
}
