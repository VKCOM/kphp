...
void RegisterKphpConfiguration::handle_KphpConfiguration_class(ClassPtr klass) {
  kphp_error(!klass->members.has_any_instance_method() && !klass->members.has_any_instance_var() && !klass->members.has_any_static_method()
  ...
  klass->members.for_each([this](const ClassMemberConstant &c) {
    ...
    if (c.local_name() == function_color_palette_name_) {
      ...
      kphp_error_return(0, fmt_format("Got unexpected {} constant '{}'", configuration_class_name_, c.local_name()));
    }
  });
}
...
void RegisterKphpConfiguration::parse_palette(VertexPtr const_val, function_palette::Palette &palette) {
  ...
  kphp_error_return(arr, fmt_format("{}::{} must be a constexpr array", configuration_class_name_, function_color_palette_name_));

  for (const auto &opt : arr->args()) {
    const auto ruleset_arr = VertexUtil::get_actual_value(opt).try_as<op_array>();
    kphp_error_act(ruleset_arr, fmt_format("{}::{} must contain constexpr arrays (rulesets)", configuration_class_name_, function_color_palette_name_),
                   continue);
...
void RegisterKphpConfiguration::parse_palette_ruleset(VertexAdaptor<op_array> arr, function_palette::Palette &palette) {
  ...
  for (const auto &rule_node : arr->args()) {
    ...
    kphp_error_act(pair, fmt_format("{}::{} rule must be an associative map", configuration_class_name_, function_color_palette_name_), continue);
...
void RegisterKphpConfiguration::parse_palette_rule(VertexAdaptor<op_double_arrow> pair, function_palette::Palette &palette,
                                                   function_palette::PaletteRuleset &add_to) {
  auto parse_rule_key_colors = [this, &palette](VertexPtr key) -> std::vector<function_palette::color_t> {
    ...
    kphp_error(!color_names_strings.empty(),
               fmt_format("{}::{} map keys must be non-empty constexpr strings", configuration_class_name_, function_color_palette_name_));
    auto mapper = vk::make_transform_iterator_range([&palette](auto color_name) { return palette.register_color_name(color_name); },
  ...
  auto parse_rule_value_error = [this](VertexPtr value) -> std::string {
    ...
    kphp_error(errtext_val || (ok_val && ok_val->get_string() == "1"),
               fmt_format("{}::{} map values must be constexpr strings or number 1", configuration_class_name_, function_color_palette_name_));
...
void RegisterKphpConfiguration::handle_constant_runtime_options(const ClassMemberConstant &c) {
  ...
  kphp_error_return(arr, fmt_format("{}::{} must be a constexpr array", configuration_class_name_, runtime_options_name_));
  for (const auto &opt : arr->args()) {
    auto opt_pair = VertexUtil::get_actual_value(opt).try_as<op_double_arrow>();
    kphp_error_return(opt_pair, fmt_format("{}::{} must be an associative map", configuration_class_name_, runtime_options_name_));
    const auto *opt_key = VertexUtil::get_constexpr_string(VertexUtil::get_actual_value(opt_pair->key()));
    kphp_error_return(opt_key, fmt_format("{}::{} map keys must be constexpr strings", configuration_class_name_, runtime_options_name_));
    ...
    } else if (vk::any_of_equal(*opt_key, warmup_workers_part_key_, warmup_instance_cache_elements_part_key_, warmup_timeout_sec_key_,
                                oom_handling_memory_ratio_key_, mysql_db_name_key_, job_workers_shared_memory_distribution_weights_, thread_pool_ratio_key_,
                                thread_pool_size_key_)) {
      generic_register_simple_option(opt_pair->value(), *opt_key);
    } else {
      kphp_error(0, fmt_format("Got unexpected option {}::{}['{}']", configuration_class_name_, runtime_options_name_, *opt_key));
    }
  }
}
...
void RegisterKphpConfiguration::generic_register_simple_option(VertexPtr value, vk::string_view opt_key) const noexcept {
  ...
  kphp_error_return(opt_value, fmt_format("{}::{} must be a constexpr string", configuration_class_name_, runtime_options_name_));
...
void RegisterKphpConfiguration::register_confdata_blacklist(VertexPtr value) const noexcept {
  const auto *opt_value = VertexUtil::get_constexpr_string(VertexUtil::get_actual_value(value));
  kphp_error_return(opt_value, fmt_format("{}[{}] must be a constexpr string", runtime_options_name_, confdata_blacklist_key_));

  re2::RE2 compiled_pattern{*opt_value, re2::RE2::CannedOptions::Quiet};
  kphp_error_return(compiled_pattern.ok(), fmt_format("Got bad regex pattern in {}::{}['{}']: {}", configuration_class_name_, runtime_options_name_,
...
void RegisterKphpConfiguration::register_confdata_predefined_wildcard(VertexPtr value) const noexcept {
  ...
  kphp_error_return(wildcards, fmt_format("{}[{}] must be a constexpr array", runtime_options_name_, confdata_predefined_wildcard_key_));
  for (const auto &wildcard : wildcards->args()) {
    ...
    kphp_error_return(wildcard_str_value && !wildcard_str_value->empty(),
                      fmt_format("{}[{}][] must be non empty constexpr string", runtime_options_name_, confdata_predefined_wildcard_key_));
...
void RegisterKphpConfiguration::register_net_dc_mask(VertexPtr value) const noexcept {
  ...
  kphp_error_return(dc_masks, fmt_format("{}[{}] must be a constexpr array", runtime_options_name_, net_dc_mask_key_));
  for (const auto &dc_mask_v : dc_masks->args()) {
    ...
    kphp_error_return(index_ipv4_subnet && !index_ipv4_subnet->empty(),
                      fmt_format("{}[{}][] must be non empty constexpr string", runtime_options_name_, net_dc_mask_key_));
