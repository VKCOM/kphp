...
void FinalCheckPass::check_instanceof(VertexAdaptor<op_instanceof> instanceof_vertex) {
  ...
  kphp_error(instanceof_var_type->class_type() || instanceof_var_type->ptype() == tp_mixed,
             fmt_format("left operand of 'instanceof' should be an instance or mixed, but passed {}", instanceof_var_type->as_human_readable()));
}
...
void FinalCheckPass::check_op_func_call(VertexAdaptor<op_func_call> call) {
  if (call->func_id->is_extern()) {
    ...
    if (call->func_id->is_stub) {
      kphp_error(!G->settings().forbid_stubs_using.get(), fmt_format("try call {} stub that is prohibited by option", function_name));
    }
  }
...
void FinalCheckPass::check_serialized_fields_hierarchy(ClassPtr klass) {
  ...
  klass->members.for_each([&](ClassMemberInstanceField& f) {
    ...
    while (the_klass) {
      auto same_numbered_field =
          the_klass->members.find_member([&f_tag](const ClassMemberInstanceField& f) { return (f.serialization_tag == f_tag) && (f_tag != -1); });

      if (same_numbered_field) {
        kphp_error_return(false,
                          fmt_format("kphp-serialized-field: field with number {} found in both classes {} and {}", f_tag, the_klass->name, klass->name));
      }
