...
VertexPtr InlineSimpleFunctions::on_enter_vertex(VertexPtr root) {
  ...
  if (auto var_vertex = root.try_as<op_var>()) {
    if (in_param_list_ && var_vertex->var_id && !var_vertex->var_id->marked_as_const && !var_vertex->var_id->is_read_only &&
        !var_vertex->var_id->is_reference && !tinf::get_type(var_vertex->var_id)->is_primitive_type()) {
      inline_is_possible_ = false;
    }
    return root;
  }
