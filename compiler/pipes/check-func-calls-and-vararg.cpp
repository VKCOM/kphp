...
VertexPtr CheckFuncCallsAndVarargPass::on_func_call(VertexAdaptor<op_func_call> call) {
  ...
  if (current_function->modulite != f->modulite) {
    ...
    if (f->type == FunctionData::func_local && !is_instance_call) {
      // The main idea is that we have inline functions that generated by compiler
      // and we can manage their scope
      if (f->modifiers.is_static() && current_function->class_id == f->context_class) {
        FunctionPtr searched_f; // true function scope

        f->context_class->members.for_each([&f, &searched_f](const ClassMemberStaticMethod &m) {
        // if function was inlined from parent - this function will have the same hash in call context inlined function and this function in parent
        // otherwise this function has been inlined from the same class, we should take this scope call
        const auto *hash_context_f = m.function->return_typehint;
        if (m.function->is_inline && hash_context_f != nullptr) {

          const auto *inlined_parent_member = f->context_class->parent_class->members.find_member([&hash_context_f](const ClassMemberStaticMethod &m) {
          return m.function->return_typehint == hash_context_f;
          });

          std::string name_inlined_parent_member = std::string(inlined_parent_member->local_name()) + "$";
          auto input_f = f->local_name();

          if (input_f.starts_with(name_inlined_parent_member)) {
            searched_f = inlined_parent_member->function;
          }
        } else {
          searched_f = m.function;
        }
        });

        f = G->get_function(searched_f->name);
      }
