...
std::string get_description_for_help_impl(VertexPtr expr) {
  ...
  case op_var:
    if (const auto* constexpr_str = VertexUtil::get_constexpr_string(expr)) {
      ...
      std::replace_if(raw_str.begin(), raw_str.end(), [](char c) { return iscntrl(c) || vk::any_of_equal(c, '"', '\'', '\\'); }, '?');
...
void AnalyzePerformance::trigger_array_merge(VertexPtr first_arg) noexcept {
  ...
  auto message = "expression " + first_arg_help + " = array_merge(" + first_arg_help + ", <...>) " + "can be replaced with array_merge_into(" + first_arg_help +
                 ", <...>)";
