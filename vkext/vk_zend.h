...
#define VK_ALLOC_ZVAL(z) (z) = (zval *)emalloc(sizeof(zval))

#define VK_INIT_ZVAL(z) memset(&z, 0, sizeof(zval));
...
#define VK_ZEND_HASHES_INTERATE_VAL(_ht, _ppos, _pval)                                                                                                         \
  for (zend_hash_internal_pointer_reset_ex(_ht, _ppos); (((_pval) = zend_hash_get_current_data_ex((_ht), (_ppos))) != NULL);                                   \
...
#define VK_ZEND_HASHES_FOREACH_KEY_VAL(_ht, _h, _pzs, _val)                                                                                                    \
  do {                                                                                                                                                         \
    ...
    (_pzs) = (zend_hash_get_current_key_ex((_ht), &(_pzs), &(_h), &_pos) == HASH_KEY_IS_STRING) ? (_pzs) : NULL;
