...
void init_global_confdata_confdata() {
  ...
  confdata_sample_storage[string{"_one dot."}] =
    array<mixed>{std::make_pair(mixed{string{"one_1"}}, mixed{string{"one_value_1"}}), std::make_pair(mixed{string{"one_2"}}, mixed{string{"one_value_2"}}),
                 std::make_pair(mixed{3}, mixed{string{"one_value_3"}}), std::make_pair(mixed{string{"one_4"}}, mixed{string{"one_value_4"}})};
...
TEST(confdata_functions_test, test_confdata_get_value_unknown_key) {
  ...
  for (auto unknown_key :
       {"", "unknown_key", "_key_3", "_one dot.1", "_one dot.one_3", "_one dot.one_5", "_two dot.a.one_3a", "_two dot.b.one_3b", "_two dot.c.one_1c"}) {
...
TEST(confdata_functions_test, test_confdata_get_values_by_unknown_wildcard) {
  ...
  for (auto unknown_wildcard :
       {"1", "k", "_tx", "_one dot x.", "_two dot.c.", "_two dot.a.x", "1*", "k*", "_tx*", "_one dot x.*", "_two dot.c.*", "_two dot.a.x*"}) {
...
TEST(confdata_functions_test, test_confdata_get_values_by_any_wildcard) {
  init_global_confdata_confdata();
  ASSERT_TRUE(equals(f$confdata_get_values_by_any_wildcard(string{"_key_1"}), array<mixed>{std::make_pair(mixed{string{""}}, mixed{string{"value_1"}})}));

  ASSERT_TRUE(equals(f$confdata_get_values_by_any_wildcard(string{"_key"}), array<mixed>{std::make_pair(mixed{string{"_1"}}, mixed{string{"value_1"}}),
                                                                                         std::make_pair(mixed{string{"_2"}}, mixed{string{"value_2"}})}));

  ASSERT_TRUE(equals(f$confdata_get_values_by_any_wildcard(string{"_one dot."}),
                     array<mixed>{std::make_pair(mixed{string{"one_1"}}, mixed{string{"one_value_1"}}),
                                  std::make_pair(mixed{string{"one_2"}}, mixed{string{"one_value_2"}}), std::make_pair(mixed{3}, mixed{string{"one_value_3"}}),
