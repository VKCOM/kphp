...
TEST(array_test, find_no_mutate_in_empy_array) {
  ...
  ASSERT_EQ(arr.find_no_mutate(var{}), arr.end());
...
TEST(array_test, find_no_mutate_in_array_vector) {
  ...
  ASSERT_TRUE(equals(it.get_key(), var{4}));
  ASSERT_EQ(it.get_value(), 4);

  it = arr.find_no_mutate(var{6});
  ASSERT_NE(it, arr.end());
  ASSERT_FALSE(it.is_string_key());
  ASSERT_TRUE(equals(it.get_key(), var{6}));
  ASSERT_EQ(it.get_value(), 6);

  it = arr.find_no_mutate(8);
  ASSERT_NE(it, arr.end());
  ASSERT_FALSE(it.is_string_key());
  ASSERT_TRUE(equals(it.get_key(), var{8}));
...
TEST(array_test, find_no_mutate_in_array_map) {
  array<int> arr{
    std::make_pair(var{string{"key_1"}}, 1),
    std::make_pair(var{string{"key_2"}}, 2),
    std::make_pair(var{3}, 3),
    std::make_pair(var{string{"4"}}, 4)
  };
  const auto arr_copy = arr;
  ...
  ASSERT_NE(it, arr.end());
  ASSERT_FALSE(it.is_string_key());
  ASSERT_TRUE(equals(it.get_key(), var{3}));
