...
TEST(unsynchronized_pool_resource_test, simple_allocation) {
  ...
  mem_stats = resource.get_memory_stats();
  ASSERT_EQ(mem_stats.real_memory_used, 16);
  ASSERT_EQ(mem_stats.memory_used, 16);
  ASSERT_EQ(mem_stats.max_real_memory_used, 16);
  ASSERT_EQ(mem_stats.max_memory_used, 16);
  ASSERT_EQ(mem_stats.memory_limit, some_memory.size());
  ASSERT_EQ(mem_stats.defragmentation_calls, 0);
  ASSERT_EQ(mem_stats.huge_memory_pieces, 0);
  ASSERT_EQ(mem_stats.small_memory_pieces, 0);
  ...
  mem_stats = resource.get_memory_stats();
  ASSERT_EQ(mem_stats.real_memory_used, 16 + 1024*64);
  ASSERT_EQ(mem_stats.memory_used, 16 + 1024*64);
  ASSERT_EQ(mem_stats.max_real_memory_used, 16 + 1024*64);
  ASSERT_EQ(mem_stats.max_memory_used, 16 + 1024*64);
  ASSERT_EQ(mem_stats.memory_limit, some_memory.size());
  ASSERT_EQ(mem_stats.defragmentation_calls, 0);
  ASSERT_EQ(mem_stats.huge_memory_pieces, 0);
  ASSERT_EQ(mem_stats.small_memory_pieces, 0);
...
TEST(unsynchronized_pool_resource_test, fallback_pool_with_defragmentation) {
  ...
  void *mem16 = resource.allocate(16);
  ASSERT_TRUE(mem16);

  void *mem_left = resource.allocate(some_memory.size() - 65536 - 16 - 16);
  ASSERT_TRUE(mem_left);

  void *mem16_2 = resource.allocate(16);
  ASSERT_TRUE(mem16_2);

  resource.deallocate(mem65536, 65536);
  resource.deallocate(mem16, 16);
  resource.deallocate(mem_left, some_memory.size() - 65536 - 16 - 16);
  resource.deallocate(mem16_2, 16);

  // got fragmentation
  auto mem_stats = resource.get_memory_stats();
  ASSERT_EQ(mem_stats.real_memory_used, some_memory.size() - 16);
  ASSERT_EQ(mem_stats.memory_used, 0);
  ...
  // but we have 2 large piece with sizes 65536 - 16 - 16 and  65536, resource will use it
  int peace_left_size = 65536 - 16 - 16;
  ...
  // take other large piece, because previous one (65536 - 16 - 16) is over
  ...
  ASSERT_EQ(mem_stats.real_memory_used, some_memory.size() - 16 - peace_left_size);
  ...
  ASSERT_EQ(mem_stats.real_memory_used, some_memory.size() - 16 - peace_left_size - 16376);
  ASSERT_EQ(mem_stats.memory_used, 0);
  ASSERT_EQ(mem_stats.max_real_memory_used, some_memory.size());
  ASSERT_EQ(mem_stats.max_memory_used, some_memory.size());
  ASSERT_EQ(mem_stats.memory_limit, some_memory.size());
