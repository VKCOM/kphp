...
TEST(memory_ordered_chunk_list_test, add_random) {
  ...
  std::mt19937 gen(rd());
  std::uniform_int_distribution<memory_resource::size_type> dis(8, 63);

  constexpr memory_resource::size_type memory_size = 1024 * 1024;
  ...
  std::array<std::pair<char *, memory_resource::size_type>, total_chunks> chunks;
