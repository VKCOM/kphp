...
TEST(memory_ordered_chunk_list_test, add_memory_and_flush_to) {
  ...
  const size_t gap1 = memory_resource::details::align_for_chunk(510250);
  ...
  const size_t gap2 = memory_resource::details::align_for_chunk(235847);
  ...
  auto *gap2_node = gap1_node->get_next();
  ...
  auto *no_gap_node = gap2_node->get_next();
  ...
  ASSERT_FALSE(no_gap_node->get_next());
}

TEST(memory_ordered_chunk_list_test, add_random) {
  std::random_device rd;
  std::mt19937 gen(rd());
  std::uniform_int_distribution<size_t> dis(16, 63);
  ...
  for (auto chunk : chunks) {
    ...
    mem_from_list = mem_from_list->get_next();
  }
