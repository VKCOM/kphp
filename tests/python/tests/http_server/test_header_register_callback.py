...
class TestHeaderRegisterCallback(KphpServerAutoTestCase):
    ...
    def test_resumable_in_callback(self):
        self.kphp_server.http_request(uri='/test_header_register_callback?act_in_callback=resumable&port={}'.format(str(self.kphp_server.master_port)), timeout=5)
        self.kphp_server.assert_log(["test_header_register_callback/resumable_in_callback"], timeout=5)
