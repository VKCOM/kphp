...
class TestOomHandler(WebServerAutoTestCase):
    @classmethod
    ...
    def test_oom_inside_oom_handler(self):
        try:
            self.web_server.http_get("/test_oom_handler?test_case=oom_inside_oom_handler")
    ...
    def test_script_memory_dealloc(self):
        ...
        self.web_server.assert_log(["refcnt_before_unset=1"], timeout=5)

    def test_script_memory_realloc(self):
        self._generic_test("test_case=script_memory_realloc")
        self.web_server.assert_log(["realloc_allocations_cnt=1,realloc_mem_allocated=[1-9]\\d*"], timeout=5)

    def test_with_job_request(self):
        self._generic_test("test_case=with_job_request")
        self.web_server.assert_log(["job_request_succeeded=1"], timeout=5)
    ...
    def test_timeout_in_oom_handler(self):
        resp = self.web_server.http_get("/test_oom_handler?test_case=timeout_in_oom_handler")
        self.assertEqual(resp.status_code, 500)
        self.web_server.assert_log([
            "allocations_cnt=[1-9]\\d?,memory_allocated=8\\d{5},estimate_memory_usage\\(arr\\)=8\\d{5}",
            "timeout exit",
