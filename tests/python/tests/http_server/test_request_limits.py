...
class TestRequestLimits(WebServerAutoTestCase):
    @staticmethod
    ...
    def test_query_limit(self):
        ...
        resp = self.web_server.http_get(uri="/{}".format(("a" * (16 * 1024))))
        self.assertEqual(resp.status_code, 414)

        resp = self.web_server.http_get(uri="/a?{}".format("a" * 17000))
        self.assertEqual(resp.status_code, 414)

    @pytest.mark.k2_skip
    def test_headers_limit(self):
        key = self._get_rand_str(4000)
        resp = self.web_server.http_get("/test_limits", headers={key: "y"})
        self.assertEqual(resp.status_code, 200)
        self.assertEqual(resp.json()["HTTP_" + key.upper()], "y")

        resp = self.web_server.http_get(headers={"x" * 5000: "y"})
        self.assertEqual(resp.status_code, 400)

        value = self._get_rand_str(17000)
        resp = self.web_server.http_get("/test_limits", headers={"x": value})
        self.assertEqual(resp.status_code, 200)
        self.assertEqual(resp.json()["HTTP_X"], value)

        resp = self.web_server.http_get(headers={"x": "y" * 70000})
        self.assertEqual(resp.status_code, 431)

        resp = self.web_server.http_get(headers={"x" * 5000: "y" * 70000})
        self.assertEqual(resp.status_code, 400)

        uri = "/test_limits?" + self._get_rand_str(15000)
        value = self._get_rand_str(15000)
        resp = self.web_server.http_get(uri, headers={key: value})
        self.assertEqual(resp.status_code, 200)
        ...
        resp = self.web_server.http_get(uri, headers={key: value, key2: value2})
        self.assertEqual(resp.status_code, 200)
        self.assertEqual(resp.json()["HTTP_" + key.upper()], value)
    ...
    def test_post_limit(self):
        req_size = 4000
        resp = self.web_server.http_post("/test_big_post_data", data="x" * req_size)
        self.assertEqual(resp.status_code, 200)
        ...
        resp = self.web_server.http_post("/test_big_post_data", data="x" * (2 * 1024 * 1024 + 1))
        self.assertEqual(resp.status_code, 200)
