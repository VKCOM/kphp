...
RegularizeWrappersReturnT regularize_wrappers(const char *rpc_payload, int32_t actor_id, bool ignore_result) {
  ...
  const size_t new_wrapper_size{sizeof(RpcDestActorFlagsHeaders)};
  size_t cur_wrapper_size{0};
  int32_t cur_wrapper_actor_id{0};
  ...
  return {
    std::pair<RpcExtraHeaders, size_t>{extra_headers, new_wrapper_size},
    cur_wrapper_size,
