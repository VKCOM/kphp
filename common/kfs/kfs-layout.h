...
typedef enum { kfs_bzf_zlib = 0, kfs_bzf_bz2 = 1, kfs_bzf_xz = 2 } kfs_bz_format_t;
...
struct kfs_file_header {
  ...
  int copy_importance; // (C) usually 0; used to mark a good copy when another copy fails; copy with maximal value of copy_importance will be chosen as 'main',
                       // if several, then the copy with minimal copy_id; if several, then with maximal file size and minimal ctime
  ...
  char filename[32];      // (S) recommended filename; if none, slice_name might be used; seq_num and appropriate suffix might be appended if necessary (e.g.
                          // 'table17' -> 'table17.bin.000239')
