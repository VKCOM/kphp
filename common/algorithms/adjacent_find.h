...
template<typename Iter, // ForwardIterator
         typename Proj, // value_type(Iter) -> T
         typename Cmp = std::equal_to<std::invoke_result_t<Proj, typename std::iterator_traits<Iter>::reference>>>
...
template<typename Iter, // ForwardIterator
         typename Field, typename Cmp = std::equal_to<Field>>
...
template<typename Rng,
         ...
         typename Cmp = std::equal_to<std::invoke_result_t<Proj, vk::range_value_type<Rng>>>>
auto adjacent_find(Rng &&rng, Proj projector, Cmp cmp = Cmp()) {
  return vk::adjacent_find(std::begin(rng), std::end(rng), projector, cmp);
}

template<typename Rng, typename Field, typename Cmp = std::equal_to<Field>>
