...
void read_ucf(const char *filename, int ucf[0x110000][4]) {
  ...
  while (fgets(s, MAX_LINE_LENGTH, f)) {
    ...
    switch (type) {
      case 'F':
        if ((0 && c1 < PREPARE_TABLE_SIZE) || (0x1f80 <= c1 && c1 <= 0x1ffc) || 1) {
...
int main(int argc, const char *argv[]) {
  if (argc != 7) {
    fprintf(stderr, "usage: generate-unicode-functions <unicode-case-folding-filename> <unicode-general-category-filename> <unicode-NFKD-filename> "
                    "<unicode-uppercase-filename> <unicode-lowercase-filename> <output-filename>\n"
