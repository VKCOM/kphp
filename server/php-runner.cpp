...
void PhpScript::finish() noexcept {
  ...
  if (disable_access_log < 2) {
    ...
    const double avg_creation_time = (vk::singleton<MapStat>::get().create.time * 1'000'000) / vk::singleton<MapStat>::get().create.count;
    const double eff_creation_time = avg_creation_time / script_time;
    const double avg_dispose_time = (vk::singleton<MapStat>::get().dispose.time * 1'000'000) / vk::singleton<MapStat>::get().dispose.count;
    const double eff_dispose_time = avg_dispose_time / script_time;
    const double avg_emplace_time = (vk::singleton<MapStat>::get().emplace.time * 1'000'000) / vk::singleton<MapStat>::get().emplace.count;
    const double eff_emplace_time = avg_emplace_time / script_time;
    const double avg_find_time = (vk::singleton<MapStat>::get().find.time * 1'000'000) / vk::singleton<MapStat>::get().find.count;
    const double eff_find_time = avg_find_time / script_time;

    kprintf("[worked = %.3lf, net = %.3lf, script = %.3lf,"
            ...
            vk::singleton<MapStat>::get().create.time * 1000,
            avg_creation_time,
            eff_creation_time,
            vk::singleton<MapStat>::get().dispose.count,
            vk::singleton<MapStat>::get().dispose.time * 1000,
            avg_dispose_time,
            eff_dispose_time,
            vk::singleton<MapStat>::get().emplace.count,
            vk::singleton<MapStat>::get().emplace.time * 1000,
            avg_emplace_time,
            eff_emplace_time,
            vk::singleton<MapStat>::get().find.count,
            vk::singleton<MapStat>::get().find.time * 1000,
            avg_find_time,
            eff_find_time,
            buf);
  }
