...
enum state_t { st_empty, st_inited };
state_t state;
...
/** net query **/
php_net_query_packet_answer_t *php_net_query_packet(int connection_id, const char *data, int data_len, double timeout, protocol_type protocol, int extra_type) {
...
void mc_run_query(int host_num, const char *request, int request_len, int timeout_ms, int query_type, void (*callback)(const char *result, int result_len)) {
  ...
  php_net_query_packet_answer_t *res =
    php_net_query_packet(host_num, request, request_len, timeout_ms * 0.001, protocol_type::memcached, query_type | (PNETF_IMMEDIATE * (callback == nullptr)));
...
const char *net_event_t::get_description() const noexcept {
  ...
  std::visit(overloaded{
               [this](const net_events_data::rpc_answer &event) {
                 ...
                 snprintf(BUF.data(), BUF.size(), "RPC_RESPONSE: actor_id=%d, tl_function=%s(0x%08x), response_magic=0x%08x, bytes_length=%d", r->actor_or_port,
                          tl_magic_convert_to_name(r->function_magic), r->function_magic,
               ...
               [this](const net_events_data::rpc_error &event) {
                 auto *r = get_rpc_request(slot_id);
                 snprintf(BUF.data(), BUF.size(), "RPC_ERROR: actor_id=%d, tl_function_magic=0x%08x, error_code=%d, error_message=%s", r->actor_or_port,
                          r->function_magic, event.error_code, event.error_message);
               },
               ...
               [](const database_drivers::Response *) { snprintf(BUF.data(), BUF.size(), "EXTERNAL_DB_ANSWER"); },
               [](const curl_async::CurlResponse *) { snprintf(BUF.data(), BUF.size(), "CURL_ASYNC_RESPONSE"); },
             },
             data);
  return BUF.data();
}
