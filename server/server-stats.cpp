...
struct ScriptSamples : WithStatType<uint64_t> {
  enum class Key {
    ...
    script_init_time,
    script_free_time,
    http_connection_accept_time,
    http_connection_close_time,
    user_time,
    system_time,
...
EnumTable<QueriesStat> make_queries_stat(uint64_t script_queries, uint64_t long_script_queries,
                                         ...
                                         uint64_t script_init_time_ns, uint64_t script_free_time_ns, uint64_t connection_accept_time_ns, uint64_t connection_close_time_ns,
  ...
  result[QueriesStat::Key::http_connection_accept_time] = connection_accept_time_ns;
  result[QueriesStat::Key::http_connection_close_time] = connection_close_time_ns;
...
struct WorkerSharedStats : private vk::not_copyable {
  ...
  void add_request_stats(const EnumTable<QueriesStat> &queries, script_error_t error,
    ...
    sample[ScriptSamples::Key::http_connection_accept_time] = queries[QueriesStat::Key::http_connection_accept_time];
    sample[ScriptSamples::Key::http_connection_close_time] = queries[QueriesStat::Key::http_connection_close_time];
...
void ServerStats::add_request_stats(double script_time_sec, double net_time_sec, std::pair<double, double> script_init_free_time_sec,
                                    std::pair<double, double> connection_process_time_sec, double left_time_on_early_timeout_sec,
  ...
  const auto http_connection_accept_time = std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::duration<double>(connection_process_time_sec.first));
  const auto http_connection_close_time = std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::duration<double>(connection_process_time_sec.second));
  ...
  const auto queries_stat = make_queries_stat(script_queries, long_script_queries, script_time.count(),
                                              net_time.count(), script_init_time.count(), script_free_time.count(),
                                              http_connection_accept_time.count(), http_connection_close_time.count(),
  ...
  StatsHouseManager::get().add_request_stats(script_time.count(), net_time.count(), error, script_memory_stats, script_queries, long_script_queries,
                                             script_user_time.count(), script_system_time.count(),
                                             script_init_time.count(), script_free_time.count(), http_connection_accept_time.count(), http_connection_close_time.count(),
...
void write_to(stats_t *stats, const char *prefix, const WorkerAggregatedStats &agg, const WorkerSharedStats &shared) noexcept {
  ...
  stats->add_gauge_stat(ns2double(shared.total_queries_stat[QueriesStat::Key::http_connection_accept_time]), prefix, ".requests.http_connection_accept_time.total");
  stats->add_gauge_stat(ns2double(shared.total_queries_stat[QueriesStat::Key::http_connection_close_time]), prefix, ".requests.http_connection_close_time.total");
  ...
  write_to(stats, prefix, ".requests.http_connection_accept_time", agg.script_samples[ScriptSamples::Key::http_connection_accept_time], ns2double);
  write_to(stats, prefix, ".requests.http_connection_close_time", agg.script_samples[ScriptSamples::Key::http_connection_close_time], ns2double);
