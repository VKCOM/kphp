...
public:
  ...
  explicit tree_node(size_t size) noexcept : chunk_size(size) {}
  ...
  bool is_left() const noexcept { return this == parent->left; }
  ...
  bool has_red_child() const noexcept { return (left && left->color == RED) || (right && right->color == RED); }
};
...
bool memory_chunk_tree::has_memory_for(size_t size) const noexcept { return search(size, true); }

size_t memory_chunk_tree::get_chunk_size(tree_node* node) noexcept { return node->chunk_size; }
