...
public:
  ...
  class_instance(const Optional<bool>& null) noexcept { php_assert(null.value_state() == OptionalState::null_value); }

  template <class Derived, class = std::enable_if_t<std::is_base_of<T, Derived>{}>>
  class_instance(const class_instance<Derived>& d) : o(d.o) {}

  template <class Derived, class = std::enable_if_t<std::is_base_of<T, Derived>{}>>
  class_instance(class_instance<Derived>&& d) noexcept : o(std::move(d.o)) {}
  ...
  inline bool operator==(const class_instance<T>& rhs) const { return o == rhs.o; }
