...
public:
  ...
  T& val() noexcept { return value_; }

  const T& val() const noexcept { return value_; }

  bool has_value() const noexcept { return value_state_ == OptionalState::has_value; }

  bool is_null() const noexcept { return value_state_ == OptionalState::null_value; }

  bool is_false() const noexcept { return value_state_ == OptionalState::false_value; }

  OptionalState value_state() const noexcept { return value_state_; }

protected:
  template <class T1>
  OptionalBase(T1&& value, OptionalState value_state) noexcept : value_(std::forward<T1>(value)), value_state_(value_state) {}

  explicit OptionalBase(OptionalState value_state) noexcept : value_state_(value_state) {}
...
public:
  ...
  Optional(bool value) noexcept : Optional(value, OptionalState::false_value) {}

  template <class T1, class = enable_if_allowed<T1>>
  Optional(T1&& value) noexcept : Optional(std::forward<T1>(value), OptionalState::has_value) {}

  template <class T1, class = enable_if_allowed<T1>>
  Optional(const Optional<T1>& other) noexcept : Optional(other.val(), other.value_state()) {}

  template <class T1, class = enable_if_allowed<T1>>
  Optional(Optional<T1>&& other) noexcept : Optional(std::move(other.val()), other.value_state()) {}

  Optional& operator=(bool x) noexcept { return assign(x, OptionalState::false_value); }
...
private:
  Optional(bool value, OptionalState value_state) noexcept : Base(value_state) { php_assert(!value); }

  template <class T1, class = vk::enable_if_constructible<T, T1>>
  Optional(T1&& value, OptionalState value_state) noexcept : Base(std::forward<T1>(value), value_state) {}
...
public:
  ...
  Optional(bool value) noexcept : Base(value, value ? OptionalState::has_value : OptionalState::false_value) {}
