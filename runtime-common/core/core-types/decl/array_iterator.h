...
public:
  ...
  inline array_iterator(inner_type* self, list_hash_type* entry) noexcept __attribute__((always_inline)) : self_(self), entry_(entry) {}

  inline operator array_iterator<const value_type>() noexcept __attribute__((always_inline)) { return array_iterator<const value_type>(self_, entry_); }
  ...
  inline int64_t get_int_key() noexcept __attribute__((always_inline)) { return static_cast<bucket_type*>(entry_)->int_key; }

  inline int64_t get_int_key() const noexcept __attribute__((always_inline)) { return static_cast<const bucket_type*>(entry_)->int_key; }
  ...
  inline const_conditional_t<string>& get_string_key() noexcept __attribute__((always_inline)) { return static_cast<bucket_type*>(entry_)->string_key; }

  inline const string& get_string_key() const noexcept __attribute__((always_inline)) { return static_cast<const bucket_type*>(entry_)->string_key; }

  inline array_iterator& operator++() noexcept __attribute__((always_inline)) ubsan_supp("alignment") {
    entry_ = self_->is_vector() ? reinterpret_cast<list_hash_type*>(reinterpret_cast<value_type*>(entry_) + 1) : self_->next(static_cast<bucket_type*>(entry_));
    return *this;
  }

  inline array_iterator& operator--() noexcept __attribute__((always_inline)) ubsan_supp("alignment") {
    entry_ = self_->is_vector() ? reinterpret_cast<list_hash_type*>(reinterpret_cast<value_type*>(entry_) - 1) : self_->prev(static_cast<bucket_type*>(entry_));
    return *this;
  }

  inline bool operator==(const array_iterator& other) const noexcept __attribute__((always_inline)) { return entry_ == other.entry_; }

  inline bool operator!=(const array_iterator& other) const noexcept __attribute__((always_inline)) { return entry_ != other.entry_; }

  inline array_iterator& operator*() noexcept __attribute__((always_inline)) { return *this; }

  inline const array_iterator& operator*() const noexcept __attribute__((always_inline)) { return *this; }
  ...
  static inline array_iterator make_begin(std::add_const_t<array_type>& arr) noexcept __attribute__((always_inline)) {
    ...
    return arr.is_vector() ? array_iterator{arr.p, arr.p->entries()} : array_iterator{arr.p, arr.p->begin()};
  }
  ...
  static inline array_iterator make_end(array_type& arr) noexcept __attribute__((always_inline)) {
    return arr.is_vector() ? array_iterator{arr.p, reinterpret_cast<list_hash_type*>(reinterpret_cast<value_type*>(arr.p->entries()) + arr.p->size)}
