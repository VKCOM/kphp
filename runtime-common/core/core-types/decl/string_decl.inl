...
inline bool string_to_bool(const char* s, string_size_type size) { return size >= 2 || (size == 1 && s[0] != '0'); }
...
struct tmp_string {
  ...
  bool to_bool() const noexcept { return data && string_to_bool(data, size); }
...
public:
  static constexpr size_type max_size() noexcept { return ((size_type)-1 - sizeof(string_inner) - 1) / 4; }
