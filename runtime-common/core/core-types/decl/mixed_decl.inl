...
class Materialized final {
  ...
  Materialized(Mix* ptr) noexcept : slot_{ptr} {}

  Materialized(Mix&& value) noexcept : slot_(std::move(value)) {}
...
public:
  ...
  Materialized<mixed> operator[](int32_t key) {
  ...
  void set_value(int32_t key, const mixed& value) {
  ...
  const mixed get_value(int32_t key) const {
  ...
  bool& as_bool() __attribute__((always_inline)) {
    return *reinterpret_cast<bool*>(&storage_);
  }
  const bool& as_bool() const __attribute__((always_inline)) {
    return *reinterpret_cast<const bool*>(&storage_);
  }

  int64_t& as_int() __attribute__((always_inline)) {
    return *reinterpret_cast<int64_t*>(&storage_);
  }
  const int64_t& as_int() const __attribute__((always_inline)) {
    return *reinterpret_cast<const int64_t*>(&storage_);
  }

  double& as_double() __attribute__((always_inline)) {
    return *reinterpret_cast<double*>(&storage_);
  }
  const double& as_double() const __attribute__((always_inline)) {
    return *reinterpret_cast<const double*>(&storage_);
  }

  string& as_string() __attribute__((always_inline)) {
    return *reinterpret_cast<string*>(&storage_);
  }
  const string& as_string() const __attribute__((always_inline)) {
    return *reinterpret_cast<const string*>(&storage_);
  }

  array<mixed>& as_array() __attribute__((always_inline)) {
    return *reinterpret_cast<array<mixed>*>(&storage_);
  }
  const array<mixed>& as_array() const __attribute__((always_inline)) {
    return *reinterpret_cast<const array<mixed>*>(&storage_);
  }

  vk::intrusive_ptr<may_be_mixed_base> as_object() __attribute__((always_inline)) {
    return *reinterpret_cast<vk::intrusive_ptr<may_be_mixed_base>*>(&storage_);
  }
  const vk::intrusive_ptr<may_be_mixed_base> as_object() const __attribute__((always_inline)) {
    return *reinterpret_cast<const vk::intrusive_ptr<may_be_mixed_base>*>(&storage_);
  }
...
private:
  ...
  auto get_type_and_value_ptr(const InstanceClass&) {
    return std::make_pair(type::OBJECT, as_object_ptr<InstanceClass>());
  }
