...
public:
  ...
  explicit shape_node(T1 value) : value(std::move(value)) {}
};
...
class shape<std::index_sequence<Is...>, T...> : public shape_node<Is, T>... {
  ...
  static decltype(auto) get_node_impl(shape_node<tag, T1>* t) {
    return *t;
  }
  template <size_t tag, typename T1>
  static decltype(auto) get_node_impl(const shape_node<tag, T1>* t) {
    return *t;
  }
  template <size_t tag>
  decltype(auto) get_node() {
    return get_node_impl<tag>(this);
  }
  template <size_t tag>
...
public:
  ...
  explicit shape(shape_variadic_constructor_stub, Args&&... args) : shape_node<Is, T>(std::forward<Args>(args))... {}

  template <size_t tag>
  auto& get() {
    return get_node<tag>().value;
  }
  template <size_t tag>
  ...
  explicit shape(shape<std::index_sequence<Is2...>, T2...>&& other) noexcept : shape_node<Is, T>()... {
    (set(std::forward<shape_node<Is2, T2>>(other)), ...);
  }

  template <size_t... Is2, typename... T2, typename = enable_if_sequence_is_subset<Is2...>>
  shape(const shape<std::index_sequence<Is2...>, T2...>& other) noexcept : shape_node<Is, T>()... {
