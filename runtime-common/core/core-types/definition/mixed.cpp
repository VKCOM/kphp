...
mixed::mixed(const mixed& v) noexcept { copy_from(v); }

mixed::mixed(mixed&& v) noexcept { copy_from(std::move(v)); }

mixed::mixed(const Unknown& u __attribute__((unused))) noexcept { php_assert("Unknown used!!!" && 0); }

mixed::mixed(const char* s, string::size_type len) noexcept : mixed(string{s, len}) {}
...
const mixed mixed::operator+() const { return to_numeric(); }

int64_t mixed::operator~() const { return ~to_int(); }
...
bool mixed::operator!() const { return !to_bool(); }
...
bool mixed::is_scalar() const { return get_type() != type::NUL && get_type() != type::ARRAY && get_type() != type::OBJECT; }
...
const string mixed::get_type_str() const { return string(get_type_c_str()); }
...
bool mixed::empty_at(const array<mixed>::iterator& key) const { return get_value(key).empty(); }

bool mixed::empty_at(const array<mixed>::const_iterator& key) const { return get_value(key).empty(); }

bool mixed::empty() const { return !to_bool(); }
...
Materialized<mixed> mixed::operator[](double double_key) { return (*this)[static_cast<int64_t>(double_key)]; }

Materialized<mixed> mixed::operator[](const array<mixed>::const_iterator& it) { return Materialized<mixed>::WithRef(as_array()[it]); }

Materialized<mixed> mixed::operator[](const array<mixed>::iterator& it) { return Materialized<mixed>::WithRef(as_array()[it]); }
...
mixed mixed::set_value_return(const array<mixed>::iterator& key, const mixed& val) { return (*this)[key] = val; }

mixed mixed::set_value_return(const array<mixed>::const_iterator& key, const mixed& val) { return (*this)[key] = val; }
...
void mixed::set_value(double double_key, const mixed& value) { set_value(static_cast<int64_t>(double_key), value); }

void mixed::set_value(const array<mixed>::const_iterator& it) { return as_array().set_value(it); }

void mixed::set_value(const array<mixed>::iterator& it) { return as_array().set_value(it); }
...
const mixed mixed::get_value(double double_key) const { return get_value(static_cast<int64_t>(double_key)); }

const mixed mixed::get_value(const array<mixed>::const_iterator& it) const { return as_array().get_value(it); }

const mixed mixed::get_value(const array<mixed>::iterator& it) const { return as_array().get_value(it); }
...
bool mixed::isset(double double_key) const { return isset(static_cast<int64_t>(double_key)); }
...
void mixed::unset(double double_key) { unset(static_cast<int64_t>(double_key)); }
...
mixed operator+(const string& lhs) { return lhs.to_numeric(); }

int64_t operator&(const mixed& lhs, const mixed& rhs) { return lhs.to_int() & rhs.to_int(); }
...
bool operator<=(const mixed& lhs, const mixed& rhs) { return !(rhs < lhs); }
