...
string_buffer& operator<<(string_buffer& sb, double f) { return sb << string(f); }
...
string::size_type string_buffer::size() const noexcept { return static_cast<string::size_type>(buffer_end - buffer_begin); }

char* string_buffer::buffer() { return buffer_begin; }

const char* string_buffer::buffer() const { return buffer_begin; }
...
void string_buffer::append_char(char c) { *buffer_end++ = c; }

void string_buffer::reserve(int len) { reserve_at_least(len + 1); }
...
bool operator!=(string_buffer const& lhs, string_buffer const& rhs) { return !(lhs == rhs); }
