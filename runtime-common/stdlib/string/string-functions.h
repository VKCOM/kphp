...
inline string f$chr(int64_t v) noexcept { return {1, static_cast<char>(v)}; }

inline int64_t f$ord(const string& s) noexcept { return static_cast<unsigned char>(s[0]); }
...
inline string f$chop(const string& s, const string& what = StringLibConstants::get().WHAT_STR) noexcept { return f$rtrim(s, what); }
...
inline bool f$str_ends_with(const string& haystack, const string& needle) noexcept { return haystack.ends_with(needle); }
...
inline string f$vsprintf(const string& format, const array<mixed>& args) noexcept { return f$sprintf(format, args); }
...
inline int64_t f$strlen(const string& s) noexcept { return s.size(); }
...
inline string f$strtr(const string& subject, const mixed& from, const mixed& to) noexcept { return f$strtr(subject, from.to_string(), to.to_string()); }

inline string f$strtr(const string& subject, const mixed& replace_pairs) noexcept { return f$strtr(subject, replace_pairs.as_array("strtr")); }
...
struct str_concat_arg {
  ...
  str_concat_arg(const string& s) noexcept : data{s.c_str()}, size{s.size()} {}
  str_concat_arg(tmp_string s) noexcept : data{s.data}, size{s.size} {}

  tmp_string as_tmp_string() const noexcept { return {data, size}; }
};
