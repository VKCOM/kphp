...
public:
  explicit ToJsonVisitor(impl_::JsonWriter& writer, bool flatten_class, std::size_t depth) noexcept
      : writer_(writer), flatten_class_(flatten_class), depth_(flatten_class ? depth : ++depth) {}
  ...
  void set_precision(std::size_t precision) noexcept { writer_.set_double_precision(precision); }

  void restore_precision() noexcept { writer_.set_double_precision(0); }
...
private:
  ...
  void add_value(const string& value) noexcept { writer_.write_string(value); }

  void add_value(const JsonRawString& value) noexcept { writer_.write_raw_string(value.str); }

  void add_value(std::int64_t value) noexcept { writer_.write_int(value); }

  void add_value(bool value) noexcept { writer_.write_bool(value); }

  void add_value(double value) noexcept { writer_.write_double(value); }

  void add_null_value() noexcept { writer_.write_null(); }
