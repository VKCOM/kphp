...
template <class T>
struct sort_compare {
  bool operator()(const T& h1, const T& h2) const { return lt(h2, h1); }
};
...
struct sort_compare_numeric {
  ...
  bool operator()(const T& h1, const T& h2) const { return f$floatval(h1) > f$floatval(h2); }
};
...
struct sort_compare_string {
  bool operator()(const T& h1, const T& h2) const { return f$strval(h1).compare(f$strval(h2)) > 0; }
};

template <class T>
struct sort_compare_natural {
  bool operator()(const T& h1, const T& h2) const { return f$strnatcmp(f$strval(h1), f$strval(h2)) > 0; }
};

template <class T>
struct rsort_compare {
  bool operator()(const T& h1, const T& h2) const { return lt(h1, h2); }
};
