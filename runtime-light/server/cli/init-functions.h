...
inline kphp::coro::task<> finalize_cli_server() noexcept {
  ...
  if (auto expected{CO_AWAIT_TASK_ON_STACK(output_stream.write(system_buffer))}; !expected) [[unlikely]] {
  ...
  for (const auto& buffer : user_buffers) {
    if (auto expected{CO_AWAIT_TASK_ON_STACK(output_stream.write(buffer))}; !expected) [[unlikely]] {
