...
int32_t tl_parse_save_pos() { return static_cast<int32_t>(RpcInstanceState::get().rpc_buffer.pos()); }
...
void t_Long::typed_store(const t_Long::PhpType& v) { f$store_long(v); }
...
void t_Double::store(const mixed& tl_object) { f$store_double(f$floatval(tl_object)); }
...
void t_Double::typed_store(const t_Double::PhpType& v) { f$store_double(v); }
...
void t_Float::store(const mixed& tl_object) { f$store_float(f$floatval(tl_object)); }
...
void t_Float::typed_store(const t_Float::PhpType& v) { f$store_float(v); }
...
void t_String::store(const mixed& tl_object) { f$store_string(f$strval(tl_object)); }
...
void t_String::typed_store(const t_String::PhpType& v) { f$store_string(f$strval(v)); }
...
void t_Bool::store(const mixed& tl_object) { f$store_int(tl_object.to_bool() ? TL_BOOL_TRUE : TL_BOOL_FALSE); }
...
void t_Bool::typed_store(const t_Bool::PhpType& v) { f$store_int(v ? TL_BOOL_TRUE : TL_BOOL_FALSE); }
...
array<mixed> t_True::fetch() { return {}; }
