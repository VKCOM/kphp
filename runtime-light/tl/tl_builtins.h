...
template<typename SerializerT, FieldAccessType ac, typename OptionalFieldT>
inline OptionalFieldT &get_serialization_target_from_optional_field(OptionalFieldT &v) requires(!need_Optional<typename SerializerT::PhpType>::value) {
  return v;
}
...
struct t_Vector {
  ...
  void store(const mixed &v) {
    const auto &cq{get_component_context()->rpc_component_context.current_query};
...
struct t_Maybe {
  ...
  void store(const mixed &v) {
    ...
    if (name == string{"resultFalse"}) {
      ...
      get_component_context()->rpc_component_context.current_query.raise_storing_error("Constructor %s of type Maybe was not found in TL scheme", name.c_str());
    }
  }
...
struct tl_Dictionary_impl {
  ...
  void store(const mixed &v) {
    if (!v.is_array()) {
      get_component_context()->rpc_component_context.current_query.raise_storing_error("Expected array (dictionary), got something strange");
      return;
    }
...
struct t_Tuple {
  ...
  void typed_store(const PhpType &v) {
    ...
    for (int64_t i = 0; i < size; ++i) {
      if (!v.isset(i)) {
        get_component_context()->rpc_component_context.current_query.raise_storing_error("Tuple[%ld] not set", i);
        return;
      }
      store_magic_if_not_bare(inner_magic);
      elem_state.typed_store(v.get_value(i));
...
struct tl_array {
  ...
  void typed_store(const PhpType &v) {
    ...
    for (int64_t i = 0; i < size; ++i) {
      if (!v.isset(i)) {
        get_component_context()->rpc_component_context.current_query.raise_storing_error("Array[%ld] not set", i);
        return;
      }
      store_magic_if_not_bare(inner_magic);
