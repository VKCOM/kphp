...
void PhpScriptMutableGlobals::once_alloc_linear_mem(unsigned int n_bytes) {
  ...
  g_linear_mem = static_cast<char *>(RuntimeAllocator::current().alloc0_global_memory(n_bytes));
...
void PhpScriptMutableGlobals::once_alloc_linear_mem(const char *lib_name, unsigned int n_bytes) {
  ...
  libs_linear_mem[key_lib_name] = static_cast<char *>(RuntimeAllocator::current().alloc0_global_memory(n_bytes));
