...
auto InstanceState::run() noexcept -> kphp::coro::task<> {
  co_await kphp::coro::when_all(std::bind_front(&InstanceState::service_loop, this),
                                std::bind_front(&InstanceState::accept_loop, this)); // both never return  kphp::log::assertion(false);
}
