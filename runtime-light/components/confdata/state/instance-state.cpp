...
auto InstanceState::accept_loop() noexcept -> kphp::coro::task<> {
  for (;;) {
    ...
    auto callback{[](std::span<const std::byte>) noexcept {}};
    if (auto expected{co_await kphp::coro::on_stack(&kphp::component::stream::read_all<decltype(callback)>, request_stream, std::move(callback))}; !expected)
        [[unlikely]] {
