...
private:
  ...
  auto perform_update(std::span<const tl::confdata::KeyValuePair> events) noexcept -> bool;
  auto apply_events(kphp::confdata::storage& storage, kphp::confdata::storage::editor& editor,
                    std::span<const tl::confdata::KeyValuePair> events) noexcept -> bool;
};

struct InstanceState::confdata_sync_error final {
...
class InstanceState::confdata_piece final {
