...
inline std::expected<void*, int32_t> alloc_shared_memory(size_t size, size_t align) noexcept {
  void* pointer{};
  if (const auto error_code{k2_alloc_shared_memory(size, align, std::addressof(pointer))}; error_code != k2::errno_ok) [[unlikely]] {
...
inline std::expected<std::span<const std::byte>, int32_t> get_shared_memory(std::string_view name) noexcept {
  const void* pointer{};
  size_t size{};
  if (const auto error_code{k2_get_shared_memory(name.data(), name.size(), std::addressof(pointer), std::addressof(size))}; error_code != k2::errno_ok)
      [[unlikely]] {
    return std::unexpected{error_code};
  }
