...
inline auto alloc_aligned(size_t size, std::align_val_t al) noexcept -> void* {
  ...
  if (unlikely(align == 0 || !std::has_single_bit(align) || align >= kphp::coro::detail::memory::task::control_block::max_alignment())) {
  ...
  if (unlikely(size > kphp::coro::detail::memory::task::control_block::max_size() - (align - 1) - cb_size)) {
  ...
  if (total_size <= task_allocator::get().segment_size()) [[likely]] {
    base = task_allocator::get().alloc_script_memory(total_size);
  ...
  new (reinterpret_cast<kphp::coro::detail::memory::task::control_block*>(aligned_u - cb_size)) // NOLINT
      kphp::coro::detail::memory::task::control_block{.backend = backend, .base_offset = static_cast<uint16_t>(base_offset_u)};
