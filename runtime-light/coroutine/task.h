...
struct task_t : public task_base_t {
  ...
  struct promise_base_t {
    task_t get_return_object() noexcept { return task_t{std::coroutine_handle<promise_type>::from_promise(*static_cast<promise_type*>(this))}; }
    ...
    final_suspend_t final_suspend() noexcept { return final_suspend_t{}; }
    ...
    static task_t get_return_object_on_allocation_failure() { php_critical_error("cannot allocate memory for task_t"); }
  ...
  void execute() { get_handle().resume(); }
  ...
  struct awaiter_t {
    ...
    T await_resume() noexcept { return task->get_result(); }

    bool resumable() const noexcept { return task->done(); }

    void cancel() const noexcept { task->get_handle().promise().next = nullptr; }

    task_t* task;
  };

  awaiter_t operator co_await() { return awaiter_t{this}; }

  std::coroutine_handle<promise_type> get_handle() { return std::coroutine_handle<promise_type>::from_address(handle_address); }

  // conversion functions
  //
  // erase type
  explicit operator task_t<void>() && noexcept { return task_t<void>{std::coroutine_handle<>::from_address(std::exchange(handle_address, nullptr))}; }
  // restore erased type
  template <typename U>
