...
struct fork_future {
  ...
  explicit fork_future(int64_t fork_id) : awaited_fork_id(fork_id), timeout_d(0) {}
  fork_future() : awaited_fork_id(-1), timeout_d(0) {}
};
...
struct stream_future {
  ...
  explicit stream_future(uint64_t stream) : stream_d(stream), timeout_d(0) {}
  stream_future() :stream_d(0), timeout_d(0){}
};

namespace std {
template <>
struct hash<fork_future> {
  std::size_t operator()(const fork_future& f) const {
...
struct hash<stream_future> {
  std::size_t operator()(const stream_future& s) const {
