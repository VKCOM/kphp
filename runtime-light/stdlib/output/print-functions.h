...
inline void print(const char* s) noexcept { print(s, strlen(s)); }

inline void print(const string_buffer& sb) noexcept { print(sb.buffer(), sb.size()); }

inline void print(const string& s) noexcept { print(s.c_str(), s.size()); }
...
inline void f$echo(const string& s) noexcept { print(s); }

inline Optional<int64_t> f$fprintf(const mixed&, const string&, const array<mixed>&) { php_critical_error("call to unsupported function"); }

inline Optional<int64_t> f$fputcsv(const mixed&, const array<mixed>&, string = string(",", 1), string = string("\"", 1), string = string("\\", 1)) {
  php_critical_error("call to unsupported function");
}

inline int64_t f$printf(const string&, const array<mixed>&) { php_critical_error("call to unsupported function"); }
