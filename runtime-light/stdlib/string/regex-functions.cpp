...
struct RegexInfo final {
  ...
  RegexInfo(std::string_view regex_, std::string_view subject_, std::string_view replacement_,
      ...
      : regex(regex_), subject(subject_), replacement(replacement_), group_names(regex_pcre2_group_names_t::allocator_type{memory_resource_}) {}
};
...
bool replace_regex(RegexInfo& regex_info, uint64_t limit) noexcept {
  ...
  if (limit == std::numeric_limits<uint64_t>::max()) [[likely]] {
    regex_info.replace_count =
        pcre2_substitute_8(regex_info.regex_code, reinterpret_cast<PCRE2_SPTR8>(regex_info.subject.data()), regex_info.subject.size(), 0,
                           PCRE2_SUBSTITUTE_GLOBAL, nullptr, regex_state.match_context.get(), reinterpret_cast<PCRE2_SPTR8>(regex_info.replacement.data()),
                           regex_info.replacement.size(), reinterpret_cast<PCRE2_UCHAR8*>(runtime_ctx.static_SB.buffer()), std::addressof(output_length));
