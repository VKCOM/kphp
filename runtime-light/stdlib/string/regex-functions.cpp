...
struct RegexInfo final {
  ...
  RegexInfo(std::string_view regex_, std::string_view subject_, std::string_view replacement_) noexcept
      : regex(regex_), subject(subject_), replacement(replacement_) {}
};
...
bool match_regex(RegexInfo& regex_info, size_t offset) noexcept {
  ...
  // (for example, 1 if there are no captures), zero if the vector of offsets is too small, or a negative error code for no match and other
  // errors.
