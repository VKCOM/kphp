  ...
  void compile_regex(kphp::regex::details::RegexCoreState<Scope>& regex_state, string pattern, const string& subject = {}) noexcept {
    ...
    if (static_cast<bool>(compile_options & PCRE2_UTF)) {
      ...
      if (!mb_UTF8_check(subject.c_str())) [[unlikely]] {
        kphp::log::warning("invalid UTF-8 subject: {}", subject.c_str());
        return;
      }
    }
    ...
    m_re = regex_state.add_compiled_regex(std::move(pattern), this->compile_options, std::move(re))->get().regex_code;
  }
...
public:
  ...
  regexp(const regexp&) noexcept = default;
  regexp(regexp&&) noexcept = default;
  regexp& operator=(const regexp&) noexcept = default;
  ...
  void compile_time_init(string pattern) noexcept {
    compile_regex(RegexImageState::get_mutable(), std::move(pattern));
  }
...
inline bool correct_offset(int64_t& offset, std::string_view subject) noexcept {
...
inline std::optional<array<mixed>> split_regex(const kphp::pcre2::regex& re, const string& subject, int64_t limit, uint32_t match_options, bool no_empty,
...
// when calling from the coroutine, please make sure that 'count' and `opt_count` exist for the same time as the calling coroutine
...
inline bool preg_match_check_args(const string& subject, int64_t flags, int64_t& offset) noexcept {
...
inline bool preg_match_all_check_args(const string& subject, int64_t flags, int64_t& offset) noexcept {
...
Optional<int64_t> f$preg_match(string pattern, const string& subject,
                               const Optional<std::variant<std::monostate, std::reference_wrapper<mixed>>>& opt_matches = {},
                               int64_t flags = kphp::regex::PREG_NO_FLAGS, int64_t offset = 0) noexcept;
...
Optional<int64_t> f$preg_match_all(string pattern, const string& subject,
                                   const Optional<std::variant<std::monostate, std::reference_wrapper<mixed>>>& opt_matches = {},
                                   int64_t flags = kphp::regex::PREG_NO_FLAGS, int64_t offset = 0) noexcept;
...
Optional<string> f$preg_replace(string pattern, const string& replacement, const string& subject, int64_t limit = kphp::regex::PREG_NOLIMIT,
                                Optional<std::variant<std::monostate, std::reference_wrapper<int64_t>>> opt_count = {}) noexcept;
...
Optional<array<mixed>> f$preg_split(string pattern, const string& subject, int64_t limit = kphp::regex::PREG_NOLIMIT,
                                    int64_t flags = kphp::regex::PREG_NO_FLAGS) noexcept;
...
kphp::coro::task<mixed> f$preg_replace_callback(kphp::regex::regexp regex, F callback, mixed subject, int64_t limit,
  ...
  if (!subject.is_array()) {
    auto task{[](kphp::regex::regexp regex, F callback, string subject, int64_t limit,
                 Optional<std::variant<std::monostate, std::reference_wrapper<int64_t>>> opt_count,
                 int64_t flags) noexcept { return f$preg_replace_callback(regex, std::move(callback), std::move(subject), limit, opt_count, flags); }};
    co_return co_await kphp::coro::on_stack(std::move(task), regex, std::move(callback), subject.to_string(), limit, opt_count, flags);
  }
  ...
  for (const auto& it : subject_arr) {
    ...
    auto task{[](kphp::regex::regexp regex, F callback, string subject, int64_t limit,
                 Optional<std::variant<std::monostate, std::reference_wrapper<int64_t>>> opt_count,
                 int64_t flags) noexcept { return f$preg_replace_callback(regex, std::move(callback), std::move(subject), limit, opt_count, flags); }};
    if (auto replace_result{co_await kphp::coro::on_stack(std::move(task), regex, callback, it.get_value().to_string(), limit, replace_one_count, flags)};
...
kphp::coro::task<Optional<string>> f$preg_replace_callback(mixed pattern, F callback, string subject, int64_t limit,
  ...
  if (!pattern.is_array()) {
    auto task{[](string pattern, F callback, string subject, int64_t limit, Optional<std::variant<std::monostate, std::reference_wrapper<int64_t>>> opt_count,
                 int64_t flags) noexcept { return f$preg_replace_callback(pattern, std::move(callback), std::move(subject), limit, opt_count, flags); }};
    co_return co_await kphp::coro::on_stack(std::move(task), pattern.to_string(), std::move(callback), subject, limit, opt_count, flags);
  }
  ...
  for (const auto& it : pattern_arr) {
    ...
    if (auto replace_result{
            co_await kphp::coro::on_stack(std::move(task), it.get_value().to_string(), callback, std::move(result), limit, replace_one_count, flags)};
        replace_result.has_value()) [[likely]] {
      count += replace_one_count;
...
kphp::coro::task<mixed> f$preg_replace_callback(mixed pattern, F callback, mixed subject, int64_t limit,
  ...
  if (!subject.is_array()) {
    auto task{
        [](mixed pattern, F callback, string subject, int64_t limit, Optional<std::variant<std::monostate, std::reference_wrapper<int64_t>>> opt_count,
           int64_t flags) noexcept { return f$preg_replace_callback(std::move(pattern), std::move(callback), std::move(subject), limit, opt_count, flags); }};
    co_return co_await kphp::coro::on_stack(std::move(task), std::move(pattern), std::move(callback), subject.to_string(), limit, opt_count, flags);
  }
  ...
  for (const auto& it : subject_arr) {
    ...
    auto task{
        [](mixed pattern, F callback, string subject, int64_t limit, Optional<std::variant<std::monostate, std::reference_wrapper<int64_t>>> opt_count,
           int64_t flags) noexcept { return f$preg_replace_callback(std::move(pattern), std::move(callback), std::move(subject), limit, opt_count, flags); }};
    if (auto replace_result{co_await kphp::coro::on_stack(std::move(task), pattern, callback, it.get_value().to_string(), limit, replace_one_count, flags)};
        replace_result.has_value()) [[likely]] {
      count += replace_one_count;
      result.set_value(it.get_key(), std::move(replace_result.val()));
    } else {
...
auto f$preg_replace_callback(T1&& pattern, T2&& callback, T3&& subject, int64_t limit,
  ...
  auto task{[](T1&& pattern, T2&& callback, T3&& subject, int64_t limit, Optional<std::variant<std::monostate, std::reference_wrapper<int64_t>>> opt_count,
               int64_t flags) noexcept {
  ...
  co_return co_await kphp::coro::on_stack(std::move(task), std::forward<T1>(pattern), std::forward<T2>(callback), std::forward<T3>(subject), limit, opt_count,
                                          flags);
}
