...
struct Info final {
  ...
  Info(std::string_view regex_, std::string_view subject_, std::string_view replacement_) noexcept
      : regex(regex_),
        subject(subject_),
...
std::optional<std::pair<string, size_t>> replace_one(regex_pcre2_code_t code, std::string_view subject, std::string_view replacement,
                                                     const RegexInstanceState& regex_state, string_buffer& sb, PCRE2_SIZE buffer_length, size_t offset,
...
coro::task<bool> replace_callback(Info& regex_info, F callback, uint64_t limit, int64_t flags) noexcept {
  ...
  for (; limit == std::numeric_limits<uint64_t>::max() || regex_info.replace_count < limit; ++regex_info.replace_count) {
    if (!match(regex_info, match_offset)) [[unlikely]] {
      co_return false;
    }
    ...
    mixed mixed_matches;
    set_matches(regex_info, flags, mixed_matches, trailing_unmatch::skip);
    ...
    auto replace_one_result = replace_one(regex_info.regex_code, {subject.c_str(), subject.size()}, {replacement.c_str(), replacement.size()}, regex_state, sb,
...
kphp::coro::task<Optional<string>> f$preg_replace_callback(string pattern, F callback, string subject, int64_t limit = kphp::regex::PREG_NOLIMIT,
  ...
  success &= co_await kphp::regex::replace_callback(
      regex_info, std::move(callback), limit == kphp::regex::PREG_NOLIMIT ? std::numeric_limits<uint64_t>::max() : static_cast<uint64_t>(limit), flags);
  if (!success) [[unlikely]] {
    co_return Optional<string>{};
  }
