...
inline kphp::coro::task<std::optional<int64_t>> wait_queue_next(int64_t queue_id, std::chrono::nanoseconds timeout) noexcept {
  ...
  static constexpr auto wait_queue_next_task{
      [](kphp::coro::await_set<int64_t>& await_set) noexcept -> kphp::coro::task<std::optional<int64_t>> { co_return co_await await_set.next(); }};
  ...
  if (auto opt_future{*wait_result}; opt_future.has_value()) {
    ...
    fork_info.get().awaited = false; // allow further f$wait awaiting on the fork
...
inline void wait_queue_push(int64_t queue_id, int64_t fork_id) noexcept {
  ...
  await_set.push(wait_queue_wrapper_task(fork_task, fork_id));
}
...
requires(kphp::forks::is_future_queue<future_queue_t>::value)
