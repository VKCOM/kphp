...
inline kphp::coro::task<std::optional<int64_t>> wait_queue_next(int64_t queue_id, duration_type timeout) noexcept {
  ...
  auto wait_result{co_await kphp::coro::on_stack(
      [](duration_type timeout, auto wait_queue_next_task, auto& await_set) noexcept {
        return kphp::coro::io_scheduler::get().schedule(timeout, wait_queue_next_task, await_set.next());
      },
      timeout, wait_queue_next_task, await_set)};
  if (!wait_result) {
    co_return std::nullopt;
  }
...
inline kphp::coro::task<Optional<int64_t>> f$wait_queue_next(int64_t queue_id, double timeout = -1.0) noexcept {
  auto opt_result{co_await kphp::coro::on_stack(
      [](int64_t queue_id, std::chrono::nanoseconds timeout) noexcept {
        return kphp::forks::id_managed(kphp::forks::wait_queue_next<std::chrono::nanoseconds>, queue_id, timeout);
      },
