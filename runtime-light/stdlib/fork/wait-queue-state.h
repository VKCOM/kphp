...
public:
  ...
  [[nodiscard("queue id is needed for deletion")]] int64_t create_queue() noexcept {
    const int64_t wait_queue_id{++next_wait_queue_id};
    queues.emplace(wait_queue_id, kphp::coro::await_set_waiting_policy::suspend_on_empty);
  ...
  bool erase_queue(int64_t queue_id) noexcept {
    return queues.erase(queue_id) == 1;
  }
};
