...
kphp::coro::task<array<mixed>> rpc_tl_query_result_one_impl(int64_t query_id) noexcept {
  ...
  auto response_expected{co_await kphp::coro::on_stack([](auto awaiter_task) noexcept { return kphp::forks::id_managed(std::move(awaiter_task)); },
                                                       *std::exchange(opt_awaiter_task, std::nullopt))};
  ...
  static constexpr auto awaiter_coroutine{
      [](kphp::rpc::query q, int64_t query_id, bool collect_responses_extra_info) noexcept -> kphp::coro::shared_task<std::expected<string, int32_t>> {
        ...
        auto fetch_result{co_await kphp::coro::on_stack(
            [](query q, auto response_buffer_provider) noexcept {
              return kphp::coro::io_scheduler::get().schedule(kphp::rpc::query::response<decltype(response_buffer_provider)>, std::move(q),
