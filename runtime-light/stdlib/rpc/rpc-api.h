...
inline kphp::coro::task<bool> f$store_error(int64_t error_code, string error_msg) noexcept {
  ...
  if (auto expected{co_await kphp::coro::on_stack(
          [](std::span<const std::byte> response) noexcept { return kphp::forks::id_managed(kphp::rpc::send_response, response); }, tls.view())};
      !expected) [[unlikely]] {
  ...
  for (const auto& it : std::as_const(query_ids)) {
    res.set_value(
        ...
        co_await kphp::coro::on_stack(
            [](int64_t query_id) noexcept { return kphp::forks::id_managed(kphp::rpc::detail::rpc_tl_query_result_one_impl, query_id); }, it.get_value()));
  }
...
kphp::coro::task<array<array<mixed>>> f$rpc_fetch_responses(array<T> query_ids) noexcept {
  co_return co_await kphp::coro::on_stack([](array<int64_t> query_ids) noexcept { return f$rpc_fetch_responses(std::move(query_ids)); },
...
inline kphp::coro::task<array<array<mixed>>> f$rpc_fetch_responses_synchronously(array<int64_t> query_ids) noexcept {
  co_return co_await kphp::coro::on_stack([](array<int64_t> query_ids) noexcept { return f$rpc_fetch_responses(std::move(query_ids)); }, std::move(query_ids));
}

template<class T>
kphp::coro::task<array<array<mixed>>> f$rpc_fetch_responses_synchronously(array<T> query_ids) noexcept {
  co_return co_await kphp::coro::on_stack([](array<int64_t> query_ids) noexcept { return f$rpc_fetch_responses_synchronously(std::move(query_ids)); },
                                          array<int64_t>::convert_from(query_ids));
}
  ...
  for (const auto& it : std::as_const(query_ids)) {
    res.set_value(it.get_key(), co_await kphp::coro::on_stack(
                                    [](int64_t query_id) noexcept {
                                      return kphp::forks::id_managed(kphp::rpc::detail::typed_rpc_tl_query_result_one_impl, query_id, error_factory_type{});
                                    },
                                    it.get_value()));
  }
  co_return std::move(res);
