...
bool f$instance_cache_store(const string& key, class_instance<InstanceType> instance, int64_t ttl_sec = 0) noexcept {
  ...
  const uint64_t class_name_hash{InstanceType::get_class_name_hash()};
  std::memcpy(mem, std::addressof(class_name_hash), hash_size);
...
ClassInstanceType f$instance_cache_fetch(const string& class_name, const string& key, bool /* even_if_expired */ = false) noexcept {
  ...
  const auto materialize{[&class_name, &key](std::span<const std::byte> mem) noexcept -> ClassInstanceType {
    ...
    std::memcpy(std::addressof(stored_class_name_hash), mem.data(), sizeof(stored_class_name_hash));
...
inline bool f$instance_cache_update_ttl(const string& key, int64_t ttl_sec = 0) noexcept {
  if (key.empty()) [[unlikely]] {
    kphp::log::warning("instance_cache_update_ttl. empty key is not supported");
    return false;
  }
  if (ttl_sec < 0) [[unlikely]] {
    kphp::log::warning("instance_cache_update_ttl. ttl less than 0, key will be stored forever: ttl -> {}, key -> {}", ttl_sec, key.c_str());
    ttl_sec = 0;
  }
  if (constexpr int64_t max_ttl_sec{std::numeric_limits<int64_t>::max() / 1000}; ttl_sec > max_ttl_sec) [[unlikely]] {
    kphp::log::warning("instance_cache_update_ttl. ttl is too large, key will be stored forever: ttl -> {}, max ttl -> {}, key -> {}", ttl_sec, max_ttl_sec,
                       key.c_str());
    ttl_sec = 0;
  }
  // the platform expects ttl in milliseconds, while the PHP API accepts seconds
