...
private:
  struct string_8bytes {
    ...
    typename __string<Allocator>::string_inner inner{0, 0, ExtraRefCnt::for_global_const};
  ...
  private:
    ...
    constexpr single_char_storage_hack(std::index_sequence<Chars...>) noexcept
      : single_char_storage{{string_cache::string_8bytes(static_cast<char>(Chars))...}} {}
  ...
  struct constexpr_number_to_string<0, Digits...> {
    static constexpr string_8bytes create() noexcept {
      return sizeof...(Digits) ? string_8bytes{std::index_sequence<Digits...>{}} : string_8bytes{std::index_sequence<0>{}};
    }
  };
  ...
  static constexpr auto constexpr_make_ints(std::index_sequence<Ints...>) noexcept {
    ...
    return std::array<string_cache::string_8bytes, 10 * sizeof...(Ints)>{
      {constexpr_number_to_string<0 * sizeof...(Ints) + Ints>::create()..., constexpr_number_to_string<1 * sizeof...(Ints) + Ints>::create()...,
       constexpr_number_to_string<2 * sizeof...(Ints) + Ints>::create()..., constexpr_number_to_string<3 * sizeof...(Ints) + Ints>::create()...,
       constexpr_number_to_string<4 * sizeof...(Ints) + Ints>::create()..., constexpr_number_to_string<5 * sizeof...(Ints) + Ints>::create()...,
       constexpr_number_to_string<6 * sizeof...(Ints) + Ints>::create()..., constexpr_number_to_string<7 * sizeof...(Ints) + Ints>::create()...,
       constexpr_number_to_string<8 * sizeof...(Ints) + Ints>::create()..., constexpr_number_to_string<9 * sizeof...(Ints) + Ints>::create()...}};
  }

  static constexpr int64_t small_int_max() noexcept {
    return 100;
  }
  ...
  static const typename __string<Allocator>::string_inner &cached_large_int(int64_t i) noexcept {
...
public:
  static const typename __string<Allocator>::string_inner &empty_string() noexcept {
    static constexpr string_8bytes empty_string;
    return empty_string.inner;
  }

  static const typename __string<Allocator>::string_inner &cached_char(char c) noexcept {
    static constexpr single_char_storage_hack constexpr_char_cache;
    return constexpr_char_cache[static_cast<uint8_t>(c)].inner;
  }

  static const typename __string<Allocator>::string_inner &cached_int(int64_t i) noexcept {
  ...
  static constexpr int64_t cached_int_max() noexcept {
    return 10000;
  }
};
}