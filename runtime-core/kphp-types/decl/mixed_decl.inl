...
struct is_type_acceptable_for_mixed : vk::is_type_in_list<T, long, long long, int, bool, double, __runtime_core::string<Allocator>> {
};

template<typename T, typename Allocator>
struct is_type_acceptable_for_mixed<array<T, Allocator>, Allocator> : is_constructible_or_unknown<mixed<Allocator>, T> {
};
...
public:
  ...
  template<typename T, typename = std::enable_if_t<is_type_acceptable_for_mixed<std::decay_t<T>, Allocator>::value>>
  ...
  inline mixed &append(const string<Allocator> &v);
  inline mixed &append(tmp_string<Allocator> v);

  inline mixed &operator[](int64_t int_key);
  inline mixed &operator[](int32_t key) { return (*this)[int64_t{key}]; }
  inline mixed &operator[](const string<Allocator> &string_key);
  ...
  inline void set_value(const string<Allocator> &string_key, const mixed &v);
  inline void set_value(const string<Allocator> &string_key, const mixed &v, int64_t precomuted_hash);
  ...
  inline const mixed get_value(const string<Allocator> &string_key) const;
  inline const mixed get_value(const string<Allocator> &string_key, int64_t precomuted_hash) const;
  ...
  inline bool isset(const string<Allocator> &string_key, MaybeHash ...maybe_hash) const;
  ...
  template <class ...MaybeHash>
  inline void unset(const string<Allocator> &string_key, MaybeHash ...maybe_hash);
  ...
  inline const string<Allocator> to_string() const;
  inline const array<mixed, Allocator> to_array() const;
  ...
  inline string<Allocator> &as_string() __attribute__((always_inline));
  inline const string<Allocator> &as_string() const __attribute__((always_inline));

  inline array<mixed, Allocator> &as_array() __attribute__((always_inline));
  inline const array<mixed, Allocator> &as_array() const __attribute__((always_inline));
  ...
  inline const string<Allocator> &as_string(const char *function) const;
  inline const array<mixed, Allocator> &as_array(const char *function) const;
  ...
  inline string<Allocator> &as_string(const char *function);
  inline array<mixed, Allocator> &as_array(const char *function);
  ...
  inline const string<Allocator> get_type_str() const;
...
private:
  ...
  template<typename T>
  auto get_type_and_value_ptr(const array<T, Allocator>  &) { return std::make_pair(type::ARRAY  , &as_array());  }
  ...
  auto get_type_and_value_ptr(const string<Allocator>    &) { return std::make_pair(type::STRING , &as_string()); }
