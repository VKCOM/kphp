...
public:
  using value_type = T;
  using array_type = const_conditional_t<array<std::remove_const_t<T>, Allocator>>;
  ...
  inline operator array_iterator<const value_type, Allocator>() noexcept __attribute__((always_inline)) {
    return array_iterator<const value_type, Allocator>(self_, entry_);
  }
  ...
  inline const_conditional_t<string<Allocator>> &get_string_key() noexcept __attribute__((always_inline)) {
    return static_cast<bucket_type *>(entry_)->string_key;
  }

  inline const string<Allocator> &get_string_key() const noexcept __attribute__((always_inline)) {
  ...
  inline array_iterator &operator++() noexcept __attribute__((always_inline)) ubsan_supp("alignment") {
    entry_ =
      self_->is_vector() ? reinterpret_cast<list_hash_type *>(reinterpret_cast<value_type *>(entry_) + 1) : self_->next(static_cast<bucket_type *>(entry_));
    return *this;
  }

  inline array_iterator &operator--() noexcept __attribute__((always_inline)) ubsan_supp("alignment") {
    entry_ =
      self_->is_vector() ? reinterpret_cast<list_hash_type *>(reinterpret_cast<value_type *>(entry_) - 1) : self_->prev(static_cast<bucket_type *>(entry_));
    return *this;
  }
